...
& =  \sum_cE\{  Y(1) | X = 1, \mathbf{C=c} \}\text{Pr}(C=c) - \sum_cE\{  Y(0) | X = 0, \mathbf{C=c} \} \text{Pr}(C = c) \\
 \text{(By} & \text{ conditional exchangeability }\uparrow) \\
& = \sum_cE\{  Y | X = 1, \mathbf{C=c} \}\text{Pr}(C=c) - \sum_cE\{  Y | X = 0, \mathbf{C=c} \} \text{Pr}(C = c) \\
...
& =  \int_cE\{  Y(1) | X = 1, \mathbf{C=c} \}f_C(c)\text{d}c - \int_cE\{  Y(0) | X = 0, \mathbf{C=c} \} f_C(c)\text{d}c \\
& = \int_cE\{  Y | X = 1, \mathbf{C=c} \}f_C(c)\text{d}c - \int_cE\{  Y | X = 0, \mathbf{C=c} \} f_C(c)\text{d}c \\
...
所以，只有當暴露變量有兩個時，才會用到交互作用 (interaction)。
