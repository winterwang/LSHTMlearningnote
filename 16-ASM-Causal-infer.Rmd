...
The Inference lectures were orgainised and taught by Professor [Karla DiazOrdaz](https://scholar.google.com/citations?user=GSGcT-EAAAAJ&hl=en).
```
...
**傾向性評分 propensity score:** 如果研究中結果變量為 $Y$，暴露變量是二分類變量 $X$，以及需要控制的條件（混雜因素）變量 $\mathbf{C}$，那麼每個觀察對象的傾向性評分 $p(\mathbf{C})$ 可以被定義為，一個觀察對象真實被觀察到暴露變量 $X=1$ 的條件概率($p(\mathbf{C})$ is defined as the conditional probability of being exposed given covariates)：
