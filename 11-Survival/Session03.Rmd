...
summary(exp.model)
gtsummary::tbl_regression(exp.model, exponentiate = TRUE)
```
...
gtsummary::tbl_regression(weibull.model, exponentiate = FALSE)
```

- 風險度比的計算方法是 exp(-1.267 * exp(0.312)) $\approx$ 0.18; 
- 風險度比的95%信賴區間的計算方法是：
    - exp((-1.267 - 1.96*0.311) * exp(0.312)) $\approx$ 0.08
    - exp((-1.267 + 1.96*0.311) * exp(0.312)) $\approx$ 0.40
- 基線風險度的計算方法是 exp(- 2.248 * exp(0.312)) $\approx$ 0.05
