...
**白血病患者數據，評估風險比例假設**，當模型中增加了“治療組 $\times$ 時間”的交互作用項之後，它的參數估計是 $e^{\hat\gamma} = 1.008$，它的信賴區間估計是 $(0.894, 1.137)$，$p = 0.894$。似乎並沒有證據提示比例風險假設前提被違反。該數據的調整後 scaled Schoenfeld 殘差圖和事件之間關係的圖繪製在 \@ref(fig:Survfig5-1)
```
...
**AGVHD數據，評估風險比例假設**，在這個數據的例子中，我們給風險比例模型中加入了“治療組 $\times$ 時間”的交互作用項之後，發現它的參數估計是 $e^{\hat\gamma} = 0.854$，它的信賴區間估計是 $(0.726, 1.003)$，$p = 0.055$。似乎有跡象表明比例風險假設的前提沒有被滿足。
```
