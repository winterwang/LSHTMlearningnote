...
實際操作中，更常用的是把這些 Schoenfeld 殘差調整之後的 "scaled Schoenfeld residuals"。調整後的 scaled Schoenfeld residuals 的計算式是：
...
**AGVHD數據，評估風險比例假設**，在這個數據的例子中，我們給風險比例模型中加入了“治療組 $\times$ 時間”的交互作用項之後，發現它的參數估計是 $e^{\hat\gamma} = 0.854$，它的信賴區間估計是 $(0.726, 1.003)$，$p = 0.055$。似乎有跡象表明比例風險假設的前提沒有被滿足。這和我們繪製的生存曲線圖 \@ref(fig:Survfig5-2)，還有調整後 scaled Schoenfeld 殘差圖 \@ref(fig:Survfig5-3) 提示的一致。
```
...
Matingale (馬丁哥?) 殘差圖可以用來檢驗，比較連續型變量在模型中是否被正確擬合，因為有時候連續型變量需要增加該連續型變量的二次項或者多次項，也可能要用對數項之類的變形之後，才能完全把其與生存數據之間的關係完全解釋清楚。
