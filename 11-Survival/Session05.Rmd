...
其中 $\ell$ 是表示兩個模型的對數似然 (log-likelihood)，$p$ 是兩個模型中相差的預測變量的個數 (the difference in the number of parameters in the two models)。這裏模型中允許考慮變量之間的交互作用項或者是給連續型變量進行數學形式的調整（增加二次項甚至是三次，多次項）。

要注意的是，似然比檢驗顯然無法用於比較一個全參數模型（如 Weibull Model）和半參數模型（如 Cox 比例風險模型）。這是因爲他們之間不存在嵌套結構關係。
