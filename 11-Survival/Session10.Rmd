...
infile group weeks remission using ../backupfiles/gehan.raw
...
gehan <- read.table(paste0(bugpath, "/backupfiles/gehan.raw"), 
                    header =  FALSE, sep ="", 
...
```{r Surv10-03, cache=TRUE, eval=FALSE}
# TEMP: eval=FALSE until brms installed
...
```{r Surv10-04, cache=TRUE, eval=FALSE}
# TEMP: eval=FALSE until brms installed
...
```{r Surv10-fig01, fig.width = 4, fig.height = 2.5, cache=TRUE, eval=FALSE, fig.cap="The full posterior distribution of the hazard ratio from model fitCox1002", fig.align='center'}
# TEMP: eval=FALSE until brms installed
post %>% 
  transmute(`hazard ratio` = exp(b_Group2)) %>% 
...
```{r Surv10-05, cache=TRUE, eval=FALSE}
# TEMP: eval=FALSE until brms installed
fitCox1002$model
```
