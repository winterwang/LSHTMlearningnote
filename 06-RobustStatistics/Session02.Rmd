...
### 基於符號檢驗的中位數信賴區間 {#RS-s02-sign-ci}

既然我們能對中位數做假設檢驗，自然也可以把這個檢驗「翻轉 (invert)」來構造信賴區間。其邏輯十分直接：100$(1-\alpha)\%$ 信賴區間，就是所有在顯著水平 $\alpha$ 下**不被拒絕**的 $\theta_0$ 取值的集合。也就是說，我們把不同的 $\theta_0$ 代入符號檢驗，凡是 $p\geqslant 0.05$ 的 $\theta_0$ 都落在 95% 信賴區間之內。

對於上面空腹血糖的例子，用這種方式可以求出中位數的 95% 信賴區間約為 $(7.2,\, 12.5)$。不難注意到，這個區間比起用均值的 $t$ 區間要寬一些，恰好印證了符號檢驗在效能上的代價。Practical 練習中我們將親自嘗試用這個「翻轉」策略構造信賴區間。
...
### 正態近似的均值與方差 {#RS-s02-wilcox-normalapprox}

正態近似中，我們需要知道 $S^+$ 在零假設 $H_0: \theta=\theta_0$ 下的期望與方差，以便構造標準化的 $z$ 統計量。@SprentSmeeton2007 [p.50] 給出的公式如下（無相同值時）：

$$
E(S^+) = \frac{n(n+1)}{4}, \quad \text{Var}(S^+) = \frac{n(n+1)(2n+1)}{24}
$$
...
本例中 $n=12$，$E(S^+)=39$，$\text{Var}(S^+)=162.5$，所以：

```{r 06-RobustStatistic-16b}
# 與前面 Stata 示例一致：S+ = 14
S_plus <- 14
n_val <- 12
E_Splus <- n_val * (n_val + 1) / 4
Var_Splus <- n_val * (n_val + 1) * (2 * n_val + 1) / 24
z_val <- (S_plus - E_Splus) / sqrt(Var_Splus)
cat("E(S+):", E_Splus, "  Var(S+):", Var_Splus, "  z:", round(z_val, 4), "\n")
cat("Two-sided p (normal approx):", 2 * pnorm(z_val), "\n")
```

不難注意到，結果 $z \approx -1.961$，與精確法的 $p=0.0522$ 十分接近（近似法 $p=0.0499$），即便樣本量只有12，近似效果已相當令人滿意。如果數據中有同值（ties），軟體會對方差做額外的 tie 校正調整。

### Wilcoxon 符號秩和檢驗在配對數據中的應用 {#RS-s02-paired}

單樣本 $t$ 檢驗最重要的應用形式之一，就是以配對差值 (within-subject differences) 為輸入的**配對 $t$ 檢驗**。完全相同的邏輯也適用於符號檢驗和 Wilcoxon 符號秩和檢驗：計算每對觀察值的差值 $d_i$，再對這組差值進行單樣本的符號檢驗（對連續性假設下的中位數差值作推斷）或 Wilcoxon 符號秩和檢驗（對稱性假設下同時也是對均值差的推斷）。

也就是說，在配對設計裡，差值序列 $d_1, d_2, \ldots, d_n$ 替代了原始的單樣本 $x_i$，所有的檢驗步驟與前面完全一致。信賴區間的構造也沿用「翻轉」策略（Section \@ref(RS-s02-sign-ci)）：對不同的 $\theta_0$ 進行假設檢驗，確定 $p \geqslant 0.05$ 的範圍即為信賴區間。Practical 練習中的 GHQ 配對數據正是這種情境的典型例子。
...
Spearman 的秩相關 (通常用 $\rho$)，是一種基於數據排序的相關係數算法。和傳統的 Pearson 相關係數類比，是當數據無法被認定是線性相關時的另一種相關關係檢驗方法。所以秩相關不假定兩組數據之間是線性相關 (linear association)。秩相關只關心一個數據遞增時，另一個數據是否單調遞增，也就是說只要求單調 (monotonic) 關係存在即可，不要求線性。所以可以用於傾向性檢驗。
...
秩相關的相關係數爲 $0.628$。而原始數據的相關係數爲 $0.719$。不難注意到，散點圖右上角那兩個離羣值 (outliers) 對 Pearson 相關係數的拉升效果非常明顯，Spearman 秩相關對這種影響明顯更爲穩健。

除此之外，雙變量相關中還有一個常見的秩相關替代指標：Kendall $\tau$ 係數 (Kendall's tau coefficient)。它衡量的是所有成對觀察值中「一致對 (concordant pairs)」與「不一致對 (discordant pairs)」之差的比例，和 Spearman $\rho$ 在統計含義上略有不同，但在實際應用中結論往往一致。對秩相關在多變量情境下的推廣，可參考 @SprentSmeeton2007 第十章。

## 基於秩次的非參數檢驗的優缺點

優點：

- 可以讓我們拋棄很多 (正態分佈等的) 前提假設，許多真實數據本身並不能滿足這些條件，這些情況下，基於秩次的非參數檢驗能提供更高的統計效能 (power)。例如在厚尾 (heavy-tailed) 分佈下，Wilcoxon 符號秩和檢驗比單樣本 $t$ 檢驗擁有更高的效能。

缺點：

- 如果數據本身能夠滿足如正態分佈之類的假設，那麼相比較與一般的參數檢驗，基於秩次的非參數檢驗法效能就偏低。不過效能損失通常並不大，事實上在大樣本下，Wilcoxon 秩和檢驗相對於對應 $t$ 檢驗的漸進相對效能 (Asymptotic Relative Efficiency) 約為 **95.5%**，也就是說效能損失等價於只損失了大約 4.5% 的樣本量，代價相當微小。
- 基於秩次的非參數檢驗較難推廣到更加複雜的情況。
- 這些檢驗法僅僅只能幫助我們進行假設檢驗。但是多數情況下，我們更加希望能使用觀察數據對總體進行點估計 (point estimates) 並且給出信賴區間 (CIs)。通常情況下，基於秩次的非參數檢驗法就很難給出這樣的估計，雖然透過翻轉 (inversion) 的方式在原理上可行，但計算過程相對繁複（詳見 Section \@ref(RS-s02-sign-ci)）。
