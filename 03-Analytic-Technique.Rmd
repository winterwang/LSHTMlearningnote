...
1. 描述一組連續型變量的位置，location，此處的位置指的是數據分佈的**中心**位置，常用的數值是眾數 (mode)，中位數 (median)，均值 (mean)。
...
```{r diff-dispersion, echo=FALSE, fig.asp=.7, fig.width=10, fig.cap='Distributions with similar central location but different dispersion', fig.align='center', out.width='80%'}
par(mfrow=c(1,2))
set.seed(1234)
a <- rnorm(10000,50, 5)
b <- rnorm(10000, 50, 10)
hist(a, xlim=c(0,100), xlab = "Mean=50, SD=5", freq = TRUE, main = "")

hist(b, xlim=c(0,100), xlab = "Mean=50, SD=10", freq = TRUE, main = "")
```
