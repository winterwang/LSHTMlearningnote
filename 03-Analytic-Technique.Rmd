...
2.2. 四分位差 interquartile range (IQR)
...
2.3. 方差和標準差 variance and standard deviation

- 先定義每一個觀察值和均值之間的差為 $D_i = X_i - \bar{X}$。

- 由於，根據定義，$\frac{1}{n}\sum_{i=1}^n D_i=0$。

- 所以，方差 Variance 被定義為 $\frac{1}{n-1}\sum_{i=1}^n D_i^2$。

- 方差的平方根，被定義為標準差 standard deviation，$\text{SD}=\sqrt{\frac{1}{n-1}\sum_{i=1}^n D_i^2}$
