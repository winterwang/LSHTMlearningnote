...
完成這個模型之後，在 STATA 裏可以用簡便的 `estat gof, table` 命令獲取模型擬合的觀測值和期待值表格，然而 R 裏面可以用模型的擬合值和殘差來手動獲取：

```{r 09-GLM-5, warning=FALSE, cache=TRUE}
# [NOTE: LogisticDx archived from CRAN; using base R diagnostics]
# Observed vs fitted by covariate pattern
diag_df <- data.frame(
  y    = Model_perf$y,
  yhat = fitted(Model_perf)
) %>% 
  group_by(yhat) %>% 
  summarise(n = n(), y_obs = sum(y), y_exp = sum(yhat), .groups = "drop")
diag_df
# obtain Pearson's test statistics
chi2 <- sum(residuals(Model_perf, type = "pearson")^2)
