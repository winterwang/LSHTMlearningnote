...
類似的，估計截距 $\hat\alpha$ 的 $95\%$ 信賴區間的計算式便是： $\hat\alpha \pm t_{n-2, 0.975}SE(\hat\alpha)$。同樣的例子裏，$\hat\alpha=6.838, SE(\hat\beta) = 0.210, t_{188, 0.975}=1.973$。所以結局的 $95\%$ 信賴區間的計算方法就是： $6.838\pm1.973\times0.210=(6.42, 7.25)$
...
因此，這裏就引出了非常重要的一個結論，**Pearson 相關係數的平方 $r^2$ 的統計學含義是，因變量的方差 $SS_{yy}$ 中，模型的預測變量能夠解釋的部分 $1-SS_{RES}$ 的百分比。** 統計學結果的報告中，爲了和一般相關係數的意義區分，會用大寫的 $R^2$ 來表示這個模型解釋了因變量的百分比。(Section \@ref(Rsquare))
...
此處方差分析的原則是：如果複雜模型能夠更好的擬合真實實驗數據，那我們會認爲簡單模型無法解釋的大量殘差方差，有效地被複雜模型解釋了。所以，這一原則下，可以推理，複雜模型計算獲得的殘差方差，會顯著地小於簡單模型計算獲得的殘差方差。ANOVA 就提供了這個殘差方差變化的定量比較方法。
...
因此，沒有預測變量的零假設模型，它的殘差方差，就等於因變量的方差。

### 分割零假設模型的殘差方差

ANOVA，方差分析的原則，其實就是將較簡單模型 (零假設模型) 的殘差方差 $(SS_{RES_{NULL}})$，分割成下面兩個部分：

1. 替代假設的複雜模型能夠說明的模型方差  $(SS_{REG})$；
2. 替代假設的複雜模型的殘差方差 $(SS_{RES_{ALT}})$。
...
在公式 \@ref(eq:SSres-partition) 中，因變量的方差被分割成了兩個部分：$SS_{REG}$ 迴歸模型能說明的部分，和 $SS_{RES_ALT}$ 迴歸模型的殘差方差。所以，我們定義迴歸模型能說明的部分，佔因變量方差的百分比 $\frac{SS_{REG}}{SS_{yy}}$，爲決定係數 $R^2$。
...
R 輸出的結果中最下面的部分 `Multiple R-squared:  0.5408`。我們就可以用“人話”來解釋其意義：假定年齡和體重成直線關係，那麼年齡解釋了這組數據中兒童體重變化 (方差) 的 54%。

### 方差分析表格 the ANOVA table

一般情況下一個簡單線性迴歸，通過 ANOVA 對因變量方差的分割，會被彙總成下面這樣的表格：
...
表格中最右邊一列是平均平方和 (mean sum of squares)。它的定義是將方差除以各自的自由度。其中殘差的平均平方和 $MS_{RES}=\frac{SS_{RES}}{(n-2)}$ 是替代模型下殘差方差的無偏估計。總體平均平方和 (total mean sum of squares)，則是零假設模型時的殘差方差估計。在 R 裏面也已經演示過多次 `anova(model)` 是調取方差分析表格的代碼：
...
注意到 R 省略掉第三行總體方差的部分，不過其實也不太需要。檢驗統計量 F 的計算也很簡單，就是359.06320/1.62184=221.39。

### 用 ANOVA 進行假設檢驗

在 ANOVA 中使用的檢驗手段是 $F$ 檢驗。這裏用 $F$ 檢驗來比較**模型解釋的因變量方差部分** $(SS_{REG})$ 和**這個模型不能解釋的殘差方差部分** $SS_{RES}$ 經過自由度校正以後比值的大小。
...
&F=(n-2)\frac{SS_{yy}r^2}{SS_{yy}-SS_{yy}r^2}=(n-2)(\frac{r^2}{1-r^2})
\end{aligned}
$$

最後一步用到 (Section \@ref(t-r2-F)) 證明過的，迴歸係數檢驗統計量 $t$，和 Pearson 相關係數 $r$ 之間的關係。

所以，$F_{1,(n-2)}=t^2_{n-2}$
