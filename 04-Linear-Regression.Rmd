...
\text{Where } \mathbf{P} &= X(X^\prime X)^{-1}X^\prime
\end{aligned}
(\#eq:lm5-7)
$$
...
標準化殘差 (standardized residuals)，被定義爲每個觀測值的殘差和模型估計的殘差標準誤相除獲得的數據。所以符合前提條件的模型擬合後，計算的標準化殘差會服從標準正態分佈。從標準正態分佈的知識你也應該知道，**$95\%$ 的觀測值的標準化殘差必須分佈在數值 $-2, 2$ 範圍內**。另外一種標準化殘差的方法叫做內學生化殘差 (studentised residual)
