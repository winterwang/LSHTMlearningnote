...
```{r introBayes15-00-setup, include=FALSE}
# ── Guard: ensure shims + brms are available for standalone knit ──
...
d <- read.csv("../backupfiles/reedfrogs.csv", sep = ";")
str(d)
```
...
```{r introBayes15-0201-brms, cache=TRUE}
# brms equivalent: fixed intercept per tank (no pooling)
...
```{r introBayes15-03, cache=TRUE, eval=FALSE}
# [LEGACY rethinking — preserved for reference]
m13.1 <- readRDS("../Stanfits/m13_1.rds")
precis(m13.1, depth = 2)
```

```{r introBayes15-03-brms, cache=FALSE}
summary(b13.1)
```
...
```{r introBayes15-04-brms, cache=TRUE}
# brms equivalent: varying intercept per tank (partial pooling)
b13.2 <- brm(
...
```{r introBayes15-05, cache=TRUE, eval=FALSE}
# [LEGACY rethinking — preserved for reference]
m13.2 <- readRDS("../Stanfits/m13_2.rds")
compare( m13.1, m13.2 )
```

```{r introBayes15-05-brms, cache=FALSE}
b13.1 <- add_criterion(b13.1, "waic")
b13.2 <- add_criterion(b13.2, "waic")
print(loo_compare(b13.1, b13.2, criterion = "waic"))
```
...
```{r  introBayes15-06, cache=TRUE, eval=FALSE}
# [LEGACY rethinking — preserved for reference]
precis(m13.2, depth = 2, pars = c("a_bar", "sigma"))
```

```{r introBayes15-06-brms, cache=FALSE}
# Intercept ≈ a_bar; sd(tank__Intercept) ≈ sigma
posterior_summary(b13.2, pars = c("b_Intercept", "sd_tank__Intercept"))
```
...
```{r introBayes15-fig01-brms, cache=TRUE, fig.width=6, fig.height=5, fig.cap="Empirical proportions of survivors in each tadpole tank (blue), plotted with the 48 per-tank estimates from the multilevel brms model (black circles). The dashed line shows the average survival probability across tanks. Vertical lines divide tanks by initial density. Shrinkage toward the grand mean is visible, especially in smaller tanks.", fig.align='center'}
# brms equivalent: extract posterior samples
...
a_tank <- sweep(as.matrix(post[, tank_cols]), 1, post$b_Intercept, "+")

# mean survival probability per tank
...
```{r introBayes15-fig02, cache=FALSE, fig.width=6, fig.height=5,  fig.cap="The inferred population of survival across tanks. 100 Gaussian distributions of the log-odds of survival, sampled from the posteriro of b13.2.", fig.align='center'}
...
```{r  introBayes15-fig03, cache=FALSE, fig.width=6, fig.height=5,  fig.cap="Survival probabilities for 8000 new simulated tanks, averaging over the posterior distribution in previous figure.", fig.align='center'}
...
```{r introBayes15-12-brms, cache=TRUE}
# brms equivalent: varying intercepts for simulated ponds
...
```{r introBayes15-13, cache=FALSE, eval=FALSE}
# [LEGACY rethinking — preserved for reference]
m13.30 <- readRDS("../Stanfits/m13_30.rds")
precis( m13.30, depth = 2 )
```

```{r introBayes15-13-brms, cache=FALSE}
summary(b13.30)
```

下面展示所有60個水池各自的截距估計，相當於原始 `precis(depth = 2)` 的完整輸出：

```{r introBayes15-13-brms-coef, cache=FALSE}
coef(b13.30)$pond[, , "Intercept"]
```
...
```{r introBayes15-fig04-brms, cache=TRUE, fig.width=8.5, fig.height=5, fig.cap="Error of no-pooling (blue) and partial pooling (black) estimates for simulated ponds. Smaller ponds produce more error, but partial pooling estimates are better on average.", fig.align='center', message=FALSE}
# brms equivalent: extract pond-level intercepts
post_pond <- suppressWarnings(as_draws_df(b13.30))
pond_cols <- grep("^r_pond\\[", names(post_pond), value = TRUE)
a_pond_mat <- sweep(as.matrix(post_pond[, pond_cols]), 1, post_pond$b_Intercept, "+")
dsim$p_partpool <- inv_logit(colMeans(a_pond_mat))
...
一旦你的計算機已經運行好了一個模型，`m13.30`，那麼假如只需要修改模型的樣本數據，模型結構不需要改變的話，使用上述的方法會大大提升新模型的運行速度，並且保存結果在 `m13.3new` 裏面。然後你只需要使用類似的方法重新繪製新數據給出的新結果，而不需要每次再重頭運行模型本身。需要重複利用的模型運算結果已經存儲在了每個 stan 模型中的 `stanfit` 部分。只要給它新的相同變量的數據框，它就能迅速給出新的事後概率分佈結果。這是非常有用的技巧。在 `brms` 中可以用 `update()` 函數達到同樣的效果。

```{r introBayes15-14-brms, cache=TRUE}
# brms equivalent: refit with new simulated data using update()
b13.3new <- update(b13.30, newdata = dsim,
                   file = "Stanfits/brm_m13_3new")
```

```{r introBayes15-fig05, cache=FALSE, fig.width=8.5, fig.height=5,  fig.cap="New data was fed to m13.3 model and generate new posterior estimations.", fig.align='center', message=FALSE, eval=FALSE}
# [LEGACY rethinking — preserved for reference]
...
```{r introBayes15-fig05-brms, cache=TRUE, fig.width=8.5, fig.height=5, fig.cap="New simulated data fed to brms model. Blue = no-pooling error; black = partial pooling error.", fig.align='center', message=FALSE}
# brms equivalent: extract pond-level intercepts from refitted model
post_new <- suppressWarnings(as_draws_df(b13.3new))
pond_cols_new <- grep("^r_pond\\[", names(post_new), value = TRUE)
a_pond_new <- sweep(as.matrix(post_new[, pond_cols_new]), 1, post_new$b_Intercept, "+")
dsim$p_partpool <- inv_logit(colMeans(a_pond_new))
...
d <- read.csv("../backupfiles/chimpanzees.csv", sep = ";")
str(d)
```
...
```{r introBayes15-17-brms, cache=TRUE}
# brms equivalent: varying intercepts for actor + block
b13.4 <- brm(
...
```{r  introBayes15-18, cache=TRUE, eval=FALSE}
# [LEGACY rethinking — preserved for reference]
m13.4 <- readRDS("../Stanfits/m13_4.rds")
precis(m13.4, depth = 2)
```

```{r introBayes15-18-brms, cache=FALSE}
summary(b13.4)
```

下面是每隻黑猩猩 (actor) 和每個實驗區塊 (block) 各自的截距估計，相當於 `precis(depth = 2)` 的個體層級輸出：

```{r introBayes15-18-brms-coef, cache=FALSE}
coef(b13.4)$actor[, , "Intercept"]
coef(b13.4)$block[, , "Intercept"]
```
...
```{r introBayes15-fig06, cache=FALSE, fig.width=5, fig.height=8,  fig.cap="Posterior means and 89% compatibility intervals for m13.4. The greater variation across actors than blocks can be seen immediately in the a and g distributions", fig.align='center', message=FALSE, eval=FALSE}
# [LEGACY rethinking — preserved for reference]
precis_plot( precis(m13.4, depth = 2) )
```

```{r introBayes15-fig06-brms, cache=FALSE, fig.width=6, fig.height=8, fig.cap="Caterpillar plot of brms multilevel chimpanzee model b13.4. Actor variation is much larger than block variation.", fig.align='center', message=FALSE}
...
其次，觀察 `sigma_a` 和 `sigma_g` 會很容易就發現不同黑猩猩之間的變化顯然比不同天進行實驗的變化要顯著的多。這一現象可以用圖 \@ref(fig:introBayes15-fig07-brms) 展示得更加清楚。

```{r introBayes15-fig07, cache=FALSE, fig.width=6, fig.height=5.5,  fig.cap="Posterior distributions of the standard deviations of varying intercepts by actor (blue), and block (black).", fig.align='center', message=FALSE, eval=FALSE}
# [LEGACY rethinking — preserved for reference]
...
```{r introBayes15-fig07-brms, cache=FALSE, fig.width=6, fig.height=5.5, fig.cap="Posterior distributions of the standard deviations of varying intercepts by actor (blue) and block (black).", fig.align='center', message=FALSE}
# brms equivalent: compare sd_actor vs sd_block
...
```{r introBayes15-19-brms, cache=TRUE}
# brms equivalent: varying intercepts for actor only (no block)
b13.5 <- brm(
...
```{r  introBayes15-20, cache=TRUE, eval=FALSE}
# [LEGACY rethinking — preserved for reference]
m13.5 <- readRDS("../Stanfits/m13_5.rds")
compare(m13.4, m13.5)
```

```{r introBayes15-20-brms, cache=FALSE}
b13.4 <- add_criterion(b13.4, "waic")
b13.5 <- add_criterion(b13.5, "waic")
print(loo_compare(b13.4, b13.5, criterion = "waic"))
```
...
> **brms 使用者注意**: 上述關於 divergent transitions 和非中心化參數化的討論主要針對直接使用 `ulam()`/Stan 的情況。`brms` 內部已經自動處理了非中心化的參數化 (non-centered parameterization)，大多數情況下不需要使用者手動操作。如果 `brms` 模型仍然出現 divergent transitions 警告，可以增大 `adapt_delta` 參數（例如 `control = list(adapt_delta = 0.99)`）。以下的魔鬼漏斗實例我們使用 `rstan` 直接編寫 Stan 代碼來演示中心化與非中心化參數化的差異，因為這類純粹的聯合分佈採樣問題不屬於 `brms` 的回歸建模範疇。
...
```{r introBayes15-21-rstan, cache=TRUE, results='hide', warning=FALSE, message=FALSE}
# rstan equivalent: centered parameterization (devil's funnel)
stan_code_centered <- "
data { int N; }
parameters {
  real v;
...
```{r  introBayes15-22, cache=TRUE, eval=FALSE}
# [LEGACY rethinking — preserved for reference]
m13.7 <- readRDS("../Stanfits/m13_7.rds")
precis(m13.7)
```

```{r introBayes15-22-rstan, cache=FALSE, warning=FALSE}
print(summary(fit_m13.7)$summary[c("v", "x"), ])
```

看一下它可憐的採樣軌跡圖 trace plot ：

```{r introBayes15-fig08, cache=FALSE, fig.width=6, fig.height=4,  fig.cap="`traceplot(m13.7)`", fig.align='center', message=FALSE, eval=FALSE}
# [LEGACY rethinking — preserved for reference]
traceplot_ulam(m13.7)
```

```{r introBayes15-fig08-rstan, cache=FALSE, fig.width=6, fig.height=4, fig.cap="Trace plot of the centered devil's funnel model (rstan). Note the poor mixing, especially for x.", fig.align='center', message=FALSE, warning=FALSE}
bayesplot::mcmc_trace(fit_m13.7, pars = c("v", "x")) +
  theme_minimal()
```
...
```{r introBayes15-23-rstan, cache=TRUE, results='hide', warning=FALSE, message=FALSE}
# rstan equivalent: non-centered parameterization
stan_code_nc <- "
data { int N; }
parameters {
  real v;
...
```{r  introBayes15-24, cache=TRUE, eval=FALSE}
# [LEGACY rethinking — preserved for reference]
m13.7nc <- readRDS("../Stanfits/m13_7nc.rds")
precis(m13.7nc)
```

```{r introBayes15-24-rstan, cache=FALSE, warning=FALSE}
print(summary(fit_m13.7nc)$summary[c("v", "z", "x"), ])
```



```{r introBayes15-fig09, cache=FALSE, fig.width=6, fig.height=4,  fig.cap="`traceplot(m13.7nc)`", fig.align='center', message=FALSE, eval=FALSE}
# [LEGACY rethinking — preserved for reference]
traceplot_ulam(m13.7nc)
```

```{r introBayes15-fig09-rstan, cache=FALSE, fig.width=6, fig.height=4, fig.cap="Trace plot of the non-centered devil's funnel model (rstan). Much better mixing compared to the centered version.", fig.align='center', message=FALSE, warning=FALSE}
bayesplot::mcmc_trace(fit_m13.7nc, pars = c("v", "z", "x")) +
  theme_minimal()
```
...
```{r introBayes15-fig10, cache=FALSE, fig.width=6, fig.height=5,  fig.cap="The devil's funnel.", fig.align='center', message=FALSE, eval=FALSE}
# [LEGACY rethinking — preserved for reference]
...
```{r introBayes15-fig10-rstan, cache=FALSE, fig.width=6, fig.height=5, fig.cap="The devil's funnel (rstan). The funnel shape arises because when v is large and negative, x is tightly constrained near zero, but when v is positive, x can spread widely.", fig.align='center', message=FALSE}
post_nc <- rstan::extract(fit_m13.7nc)
plot(post_nc$x, post_nc$v,
     bty = "n", xlab = "x", ylab = "v",
     xlim = c(-4000, 4000), ylim = c(-12, 12))
```
...
```{r  introBayes15-26, cache=TRUE, eval=FALSE}
# [LEGACY rethinking — preserved for reference]
...
```{r introBayes15-27, cache=TRUE, eval=FALSE}
# [LEGACY rethinking — preserved for reference]
...
```{r introBayes15-29, cache=TRUE, eval=FALSE}
# [LEGACY rethinking — preserved for reference]
...
```{r introBayes15-fig11, cache=TRUE, fig.width=6, fig.height=6,  fig.cap="Comparing the centered (horizontal) and non-centered (vertical) parameerizations of the multilevel chimpanzees model, m13.4. Each point is a parameter. All but two parameters lie above the diagonal, indicating better sampling for the non-centered parameterization.", fig.align='center', eval=FALSE}
# [LEGACY rethinking — preserved for reference]
...
由於 `brms` 內部自動使用非中心化參數化，上面的對比圖不直接適用。但我們可以用 `mcmc_neff()` 來檢查 `b13.4` 各參數的有效樣本比率，確認 `brms` 的自動非中心化已經產生了健康的採樣效率：

```{r introBayes15-fig11-brms, cache=FALSE, fig.width=7, fig.height=5, fig.cap="Effective sample size ratios (neff/N) for all parameters of brms model b13.4. brms automatically uses non-centered parameterization, producing healthy sampling efficiency. Ratios near 1.0 indicate efficient sampling.", fig.align='center'}
mcmc_neff(neff_ratio(b13.4)) +
  yaxis_text(hjust = 1) +
  theme_minimal()
```
...
```{r introBayes15-brms-diagnostics, cache=FALSE, fig.width=7, fig.height=4, fig.cap="Trace plots for the population-level parameters and group-level standard deviations of brms model b13.4.", fig.align='center'}
