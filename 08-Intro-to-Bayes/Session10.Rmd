```{r session10-setup, include=FALSE}
if (!exists("PI", mode = "function")) source("../R/global_setup.R", chdir = FALSE)
dir.create("Stanfits", showWarnings = FALSE)
# var2: population variance (n denominator, not n-1) — used by R² calculation
var2 <- function(x) {
  m <- mean(x)
  mean((x - m)^2)
}
# quap_link: replicate rethinking::link() via MAP + Hessian (Laplace approx).
# brms MCMC gives honest wide posteriors, but the book uses quap's artificially
# tight CIs to illustrate overfitting's "false confidence" — shade shrinks as
# polynomial degree increases, even though the model is wildly overfit.
quap_link <- function(data, degree, mass_seq, n = 1000) {
  X <- sapply(0:degree, function(d) data$mass_std^d)
  X_pred <- sapply(0:degree, function(d) mass_seq^d)
  nll <- function(par) {
    beta <- par[1:(degree + 1)]
    log_sigma <- par[degree + 2]
    mu <- X %*% beta
    -sum(dnorm(data$brain_std, mu, exp(log_sigma), log = TRUE)) -
      dnorm(par[1], 0.5, 1, log = TRUE) -
      sum(dnorm(par[2:(degree + 1)], 0, 10, log = TRUE)) -
      dnorm(log_sigma, 0, 1, log = TRUE)
  }
  init <- c(rep(0, degree + 1), 0); init[1] <- 0.5
  fit <- optim(init, nll, method = "BFGS", hessian = TRUE)
  beta_idx <- 1:(degree + 1)
  vcov_beta <- solve(fit$hessian)[beta_idx, beta_idx]
  beta_map <- fit$par[beta_idx]
  MASS::mvrnorm(n, beta_map, vcov_beta) %*% t(X_pred)
}
```
...
The rethinking model uses `exp(log_sigma)` parameterisation, which is equivalent to a log-normal prior on $\sigma$. brms supports this directly:

```{r introBayes10-03-brm, cache=TRUE, results='hide', message=FALSE}
brm_m7.1 <- brm(
  brain_std ~ mass_std,
  data = d,
  prior = c(
  ...
  file = "Stanfits/brm_m7_1"
)
```

```{r introBayes10-03-brm-summary, cache=FALSE}
summary(brm_m7.1)
...
```{r introBayes10-04, eval=FALSE}
# [LEGACY rethinking — preserved for reference]
set.seed(12)
s <- rethinking::sim( m7.1 )
...
```{r introBayes10-04-brm, cache=TRUE}
set.seed(12)
s <- posterior_predict(brm_m7.1)
r <- apply(s, 2, mean) - d$brain_std
resid_var <- var2(r)
outcome_var <- var2(d$brain_std)
1 - resid_var / outcome_var
...
```{r introBayes10-05-brm, cache=TRUE}
R2_is_bad <- function(brm_fit) {
  set.seed(12)
  s <- posterior_predict(brm_fit)
  r <- apply(s, 2, mean) - d$brain_std
  1 - var2(r) / var2(d$brain_std)
}
...
```{r introBayes10-06-brm, cache=TRUE, results='hide', message=FALSE}
brm_m7.2 <- brm(
  ...
  file = "Stanfits/brm_m7_2"
)
```

```{r introBayes10-06-brm-summary, cache=FALSE}
summary(brm_m7.2)
```
...
```{r introBayes10-07-brm, cache=TRUE, results='hide', message=FALSE}
brm_m7.3 <- brm(
...
mass_seq <- seq( from = min(d$mass_std), to = max(d$mass_std), 
                 length.out = 100)
l <- quap_link(d, degree = 1, mass_seq)
mu <- apply(l, 2, mean)
ci <- apply(l, 2, PI)
...
mass_seq <- seq( from = min(d$mass_std), to = max(d$mass_std), 
                 length.out = 100)
l <- quap_link(d, degree = 2, mass_seq)
mu <- apply(l, 2, mean)
ci <- apply(l, 2, PI)
...
mass_seq <- seq( from = min(d$mass_std), to = max(d$mass_std), 
                 length.out = 100)
l <- quap_link(d, degree = 3, mass_seq)
mu <- apply(l, 2, mean)
ci <- apply(l, 2, PI)
...
mass_seq <- seq( from = min(d$mass_std), to = max(d$mass_std), 
                 length.out = 100)
l <- quap_link(d, degree = 4, mass_seq)
mu <- apply(l, 2, mean)
ci <- apply(l, 2, PI)
...
mass_seq <- seq( from = min(d$mass_std), to = max(d$mass_std), 
                 length.out = 100)
l <- quap_link(d, degree = 5, mass_seq)
mu <- apply(l, 2, mean)
ci <- apply(l, 2, PI)
...
mass_seq <- seq( from = min(d$mass_std), to = max(d$mass_std), 
                 length.out = 100)
l <- quap_link(d, degree = 6, mass_seq)
mu <- apply(l, 2, mean)
ci <- apply(l, 2, PI)
...
Here we re-simulate the plant growth data and fit the three models from Chapter \@ref(posttreatbias) using brms non-linear formulas. These models use a multiplicative growth ratio $p$, where $h_1 = h_0 \times p$:

```{r introBayes10-11-data, cache=TRUE}
set.seed(71)
N <- 100
h0 <- rnorm(N, 10, 2)
treatment <- rep(0:1, each = N/2)
...
```{r introBayes10-11-brm, cache=TRUE, results='hide', message=FALSE}
brm_m6.6_ch7 <- brm(
...
brm_m6.8_ch7 <- brm(
  ...
  file = "Stanfits/brm_m6_8_ch7"
)
```

```{r introBayes10-11-waic, cache=FALSE}
brm_m6.7_ch7 <- add_criterion(brm_m6.7_ch7, "waic")
waic(brm_m6.7_ch7)
```
...
```{r  introBayes10-12, eval=FALSE}
# [LEGACY rethinking — preserved for reference]
set.seed(77)
rethinking::compare( m6.6, m6.7, m6.8 , func = WAIC)
# rethinking::compare( m6.6, m6.7, m6.8 , func = PSIS)
```

```{r introBayes10-12-brm, cache=FALSE}
brm_m6.6_ch7 <- add_criterion(brm_m6.6_ch7, "waic")
brm_m6.8_ch7 <- add_criterion(brm_m6.8_ch7, "waic")
loo_compare(brm_m6.6_ch7, brm_m6.7_ch7, brm_m6.8_ch7, criterion = "waic")
```
