...
mu.samples <- sapply(weight.seq, function(w) post$a + post$b1 * w + post$b2 * w^2)

# Summaries for mu
mu.mean <- apply(mu.samples, 2, mean)
mu.PI   <- apply(mu.samples, 2, PI, prob = 0.89)

# Manual posterior predictive simulation (height) using sigma samples
sim.height <- sapply(seq_along(weight.seq), function(j) {
  rnorm(length(post$a), mu.samples[, j], post$sigma)
})
height.PI <- apply(sim.height, 2, PI, prob = 0.89)

# (Optional) retain original link-based mu for comparison if link() supports data argument
# try({ mu.link <- link(m4.5, data = pred_dat) }, silent = TRUE)
...
# Posterior samples from cubic polynomial model
post_cubic <- extract.samples(m4.6)

# Build mu samples for each standardized weight value
mu.samples.cubic <- sapply(weight.seq, function(w) {
  post_cubic$a + post_cubic$b1 * w + post_cubic$b2 * w^2 + post_cubic$b3 * w^3
})
...
sim.height.cubic <- sapply(seq_along(weight.seq), function(j) {
  rnorm(length(post_cubic$a), mu.samples.cubic[, j], post_cubic$sigma)
})
