...
## Manual posterior predictive simulation replacing sim(m4.3, data=...) which
## failed due to unused 'data' argument in current rethinking version.
## We construct mu for each posterior sample and weight, then simulate heights.

post <- extract.samples(m4.3)                # list of posterior samples (a, b, sigma)
n_samp <- length(post$a)

# Compute posterior samples of mu for each weight value
mu.samples <- sapply(weight.seq, function(w) post$a + post$b * (w - xbar))

# Recompute summaries if not available or mismatched
if(!exists("mu.mean") || length(mu.mean) != length(weight.seq)) {
  mu.mean <- apply(mu.samples, 2, mean)
}
if(!exists("mu.PI") || ncol(mu.PI) != length(weight.seq)) {
  mu.PI <- apply(mu.samples, 2, PI, prob = 0.89)
}

# Posterior predictive heights: draw one simulated height for each posterior sample and weight
...
# Shaded 89% interval for mu (narrow band)
shade(mu.PI, weight.seq)

# Shaded 89% predictive interval for heights (wider band)
