```{r brms-setup-guard-s16, include=FALSE}
...
# LKJ correlation sampler (K=2 only) — replaces rethinking::rlkjcorr
# For K=2, marginal rho ~ 2*Beta(eta, eta) - 1  (exact)
rlkjcorr <- function(n, K = 2, eta = 1) {
  stopifnot(K == 2)
  rho <- 2 * rbeta(n, eta, eta) - 1
  out <- array(NA_real_, dim = c(n, K, K))
  out[,1,1] <- 1; out[,2,2] <- 1
  out[,1,2] <- rho; out[,2,1] <- rho
  out
}
```
...
# [ORIGINAL — rethinking ulam, kept for reference]
set.seed(867530)
m14.1 <- ulam(
  alist(
...
```{r introBayes16-11b, cache=TRUE, message=FALSE, results='hide'}
# [MIGRATED 2026-04] ulam() → brm() with varying intercepts + slopes per cafe
m14.1_brm <- brm(
  wait ~ afternoon + (1 + afternoon | cafe),
  data = d,
  family = gaussian(),
  prior = c(
    prior(normal(5, 2),    class = Intercept),
    prior(normal(-1, 0.5), class = b, coef = afternoon),
    prior(exponential(1),  class = sigma),
    prior(exponential(1),  class = sd),
    prior(lkj(2),          class = cor)
  ),
  chains = 4, cores = 4, seed = 867530,
  file = "Stanfits/m14.1_brm"
)
print(summary(m14.1_brm))
```
...
# [MIGRATED 2026-04] readRDS(ulam) + extract.samples() → as_draws_df(brm)
post <- suppressWarnings(as_draws_df(m14.1_brm))

dens( post$cor_cafe__Intercept__afternoon, xlim = c(-1, 1), 
       bty = "n")                      # posterior
...
# [MIGRATED 2026-04] extract.samples() → coef()/fixef()/as_draws_df()
post    <- suppressWarnings(as_draws_df(m14.1_brm))
a2 <- coef(m14.1_brm)$cafe[, "Estimate", "Intercept"]
b2 <- coef(m14.1_brm)$cafe[, "Estimate", "afternoon"]
...
Mu_est <- c( fixef(m14.1_brm)["Intercept","Estimate"],
             fixef(m14.1_brm)["afternoon","Estimate"] )
rho_est <- mean(post$cor_cafe__Intercept__afternoon)
sa_est <- mean( post$sd_cafe__Intercept )
sb_est <- mean( post$sd_cafe__afternoon )
