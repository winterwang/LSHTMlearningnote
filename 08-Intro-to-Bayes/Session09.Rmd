...
> **提醒**：本章中原始的 `quap()` 代碼（二次方近似）保留展示但不執行，改用 `brms`（HMC/MCMC）實現。由於方法論不同，數值結果可能存在細微差異，詳見 [Session 06](Session06.html) 的說明。
...
In brms, this model is a standard multiple regression of `height` on both leg lengths:

```{r introBayes09-02-brm, cache=TRUE, results='hide', message=FALSE}
brm_m6.1 <- brm(
  ...
  file = "Stanfits/brm_m6_1"
)
```

```{r introBayes09-02-brm-summary, cache=FALSE}
summary(brm_m6.1)
```
...
mcmc_intervals(as.matrix(brm_m6.1),
               pars = c("b_Intercept", "b_leg_left", "b_leg_right", "sigma"))
```
...
post <- suppressWarnings(as_draws_df(brm_m6.1))
plot(b_leg_left ~ b_leg_right, data = post, col = col.alpha(rangi2, 0.1),
     pch = 16, xlab = "br", ylab = "bl")
```
...
sumblbr <- post$b_leg_left + post$b_leg_right
...
```{r introBayes09-03-brm, cache=TRUE, results='hide', message=FALSE}
brm_m6.2 <- brm(
  height ~ leg_left,
  data = d,
  prior = c(
  ...
  file = "Stanfits/brm_m6_2"
)
```

```{r introBayes09-03-brm-summary, cache=FALSE}
summary(brm_m6.2)
```
...
```{r introBayes09-05-brm, cache=TRUE, results='hide', message=FALSE}
# kcal.per.g regressed on perc.fat
brm_m6.3 <- brm(
  K ~ F,
  data = d,
  prior = c(
    ...
    prior(normal(0, 0.5), class = "b", coef = "F"),
    prior(exponential(1), class = "sigma")
  ),
  file = "Stanfits/brm_m6_3"
)
...
brm_m6.4 <- brm(
  ...
  prior = c(
    prior(normal(0, 0.2), class = "Intercept"),
    prior(normal(0, 0.5), class = "b", coef = "L"),
    prior(exponential(1), class = "sigma")
  ),
  file = "Stanfits/brm_m6_4"
)
```

```{r introBayes09-05-brm-summary, cache=FALSE}
summary(brm_m6.3)
summary(brm_m6.4)
```
...
```{r introBayes09-06-brm, cache=TRUE, results='hide', message=FALSE}
brm_m6.5 <- brm(
  K ~ F + L,
  data = d,
  prior = c(
  ...
  file = "Stanfits/brm_m6_5"
)
```

```{r introBayes09-06-brm-summary, cache=FALSE}
summary(brm_m6.5)
```
...
In brms, this multiplicative model uses a non-linear formula. The parameter `p` represents the growth ratio $h_1 / h_0$:

```{r introBayes09-08-brm, cache=TRUE, results='hide', message=FALSE}
brm_m6.6 <- brm(
  bf(h1 ~ h0 * p, p ~ 1, nl = TRUE),
  data = d,
  prior = c(
    prior(lognormal(0, 0.25), nlpar = "p", class = "b"),
    prior(exponential(1), class = "sigma")
  ),
  file = "Stanfits/brm_m6_6"
)
```

```{r introBayes09-08-brm-summary, cache=FALSE}
summary(brm_m6.6)
```
...
In brms, the sub-model for `p` includes treatment and fungus. Because brms does not allow a `lognormal` prior with `coef=`, we use `normal(1, 0.2)` for the intercept — this is centered on the same value and closely approximates the lognormal(0, 0.2) shape near $p \approx 1$:

```{r introBayes09-09-brm, cache=TRUE, results='hide', message=FALSE}
brm_m6.7 <- brm(
  bf(h1 ~ h0 * p,
  ...
  data = d,
  prior = c(
    prior(normal(1, 0.2), nlpar = "p", class = "b", coef = "Intercept"),
    prior(normal(0, 0.5), nlpar = "p", class = "b", coef = "treatment"),
    prior(normal(0, 0.5), nlpar = "p", class = "b", coef = "fungus"),
    prior(exponential(1), class = "sigma")
  ),
  file = "Stanfits/brm_m6_7"
)
```

```{r introBayes09-09-brm-summary, cache=FALSE}
summary(brm_m6.7)
```
...
```{r introBayes09-10-brm, cache=TRUE, results='hide', message=FALSE}
brm_m6.8 <- brm(
  bf(h1 ~ h0 * p,
     p ~ 1 + treatment,
     nl = TRUE),
  data = d,
  prior = c(
    prior(normal(1, 0.25), nlpar = "p", class = "b", coef = "Intercept"),
    prior(normal(0, 0.5), nlpar = "p", class = "b", coef = "treatment"),
    prior(exponential(1), class = "sigma")
  ),
  file = "Stanfits/brm_m6_8"
)
```

```{r introBayes09-10-brm-summary, cache=FALSE}
summary(brm_m6.8)
```
...
```{r introBayes09-11-quap, eval=FALSE}
# [LEGACY rethinking — preserved for reference]
...
```{r introBayes09-11-brm, cache=TRUE, results='hide', message=FALSE}
# incorrectly included fungus
brm_m6.7b <- brm(
  bf(h1 ~ h0 * p,
     p ~ 1 + treatment + fungus,
     nl = TRUE),
...
brm_m6.8b <- brm(
  ...
  file = "Stanfits/brm_m6_8b"
)
```

```{r introBayes09-11-brm-summary, cache=FALSE}
summary(brm_m6.7b)
summary(brm_m6.8b)
```
...
# Replicate rethinking::sim_happiness() without loading the package
sim_happiness <- function(seed = 1977, N_years = 1000, max_age = 65,
                          N_births = 20, aom = 18) {
  set.seed(seed)
  H <- seq(from = -2, to = 2, length.out = N_births)
  A <- rep(1, N_births)
  M <- rep(0, N_births)
  for (t in 2:N_years) {
    A <- A + 1
    died <- which(A > max_age)
    if (length(died) > 0) { H <- H[-died]; A <- A[-died]; M <- M[-died] }
    H <- c(H, seq(-2, 2, length.out = N_births))
    A <- c(A, rep(1, N_births))
    M <- c(M, rep(0, N_births))
    for (i in seq_along(M))
      if (M[i] == 0 & A[i] >= aom) M[i] <- rbinom(1, 1, inv_logit(H[i] - 4))
  }
  data.frame(age = A, married = M, happiness = H)
}
...
In brms, the indexed intercept `a[mid]` is handled by creating a factor variable and using `0 + mid_f` to estimate separate intercepts per marriage status:

```{r introBayes09-14-brm-prep, cache=TRUE}
d2$mid <- d2$married + 1
d2$mid_f <- as.factor(d2$mid)
```

```{r introBayes09-14-brm, cache=TRUE, results='hide', message=FALSE}
brm_m6.9 <- brm(
  happiness ~ 0 + mid_f + A,
  data = d2,
  prior = c(
    prior(normal(0, 1), class = "b", coef = "mid_f1"),
    prior(normal(0, 1), class = "b", coef = "mid_f2"),
    prior(normal(0, 2), class = "b", coef = "A"),
    prior(exponential(1), class = "sigma")
  ),
  file = "Stanfits/brm_m6_9"
)
```

```{r introBayes09-14-brm-summary, cache=FALSE}
summary(brm_m6.9)
```
...
```{r introBayes09-15-brm, cache=TRUE, results='hide', message=FALSE}
brm_m6.10 <- brm(
  happiness ~ A,
  data = d2,
  prior = c(
  ...
  file = "Stanfits/brm_m6_10"
)
```

```{r introBayes09-15-brm-summary, cache=FALSE}
summary(brm_m6.10)
```
...
```{r introBayes09-18-brm, cache=TRUE, results='hide', message=FALSE}
brm_m6.11 <- brm(
  C ~ P + G,
  data = d,
  prior = c(
    prior(normal(0, 1), class = "Intercept"),
    prior(normal(0, 1), class = "b", coef = "P"),
    prior(normal(0, 1), class = "b", coef = "G"),
    prior(exponential(1), class = "sigma")
  ),
  file = "Stanfits/brm_m6_11"
)
```

```{r introBayes09-18-brm-summary, cache=FALSE}
summary(brm_m6.11)
```
...
```{r introBayes09-19-brm, cache=TRUE, results='hide', message=FALSE}
brm_m6.12 <- brm(
  C ~ P + G + U,
  data = d,
  prior = c(
    ...
    prior(normal(0, 1), class = "b", coef = "P"),
    prior(normal(0, 1), class = "b", coef = "G"),
    prior(normal(0, 1), class = "b", coef = "U"),
    prior(exponential(1), class = "sigma")
  ),
  file = "Stanfits/brm_m6_12"
)
```

```{r introBayes09-19-brm-summary, cache=FALSE}
summary(brm_m6.12)
```
