...
```{r introBayes13-07, cache=TRUE, results="hide"}
m11.4 <- ulam(
  alist(
    pulled_left ~ dbinom( 1, p ), 
...
```{r introBayes13-fig1, cache=TRUE, message=FALSE, fig.width=5.5, fig.height=4.5,  fig.cap="Posterior predictions for the Oceanic tools model. Filled points are societies with historically high contact. Open points are those with low contact. Point size is scaled by relative PSIS Pareto's k values. Larger points are more influential. The solid curve is the posterior mean for high contact societies. The dashed curve is the same for low contact societies. 89% compatibility intervals are shown by the shaded regions. (Same predictions on the natural population scale.)", fig.align='center'}
