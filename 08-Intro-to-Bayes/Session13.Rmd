...
```{r introBayes13-00-setup, cache=FALSE, include=FALSE}
...
```{r introBayes13-03, cache=TRUE, results="hide", eval=FALSE}
...
# brms replacement: sample directly from the prior
set.seed(1999) 
prior <- list(a = rnorm(10000, 0, 10))
```
...
# Now sample from a tighter prior: a ~ Normal(0, 1.5)
set.seed(1999) 
prior1.5 <- list(a = rnorm(10000, 0, 1.5))
p1 <- inv_logit( prior1.5$a )
...
# brms replacement: sample directly from priors
set.seed(1999)
prior_a <- rnorm(10000, 0, 1.5)
prior_b <- matrix(rnorm(10000 * 4, 0, 10), ncol = 4)
p <- sapply( 1:4, function(k) inv_logit(prior_a + prior_b[, k]))
...
set.seed(1999)
prior_a <- rnorm(10000, 0, 1.5)
prior_b1 <- matrix(rnorm(10000 * 4, 0, 0.5), ncol = 4)
p1 <- sapply( 1:4, function(k) inv_logit(prior_a + prior_b1[, k]))
...
# Factor variables for brms
d$actor_f     <- factor(d$actor)
d$treatment_f <- factor(d$treatment)
```
...
brm_m11.4 <- brm(
  pulled_left ~ 0 + actor_f + treatment_f,
  family = bernoulli(),
  data = d,
  prior = c(
    prior(normal(0, 1.5), class = b, coef = actor_f1),
    prior(normal(0, 1.5), class = b, coef = actor_f2),
    prior(normal(0, 1.5), class = b, coef = actor_f3),
    prior(normal(0, 1.5), class = b, coef = actor_f4),
    prior(normal(0, 1.5), class = b, coef = actor_f5),
    prior(normal(0, 1.5), class = b, coef = actor_f6),
    prior(normal(0, 1.5), class = b, coef = actor_f7),
    prior(normal(0, 0.5), class = b, coef = treatment_f2),
    prior(normal(0, 0.5), class = b, coef = treatment_f3),
    prior(normal(0, 0.5), class = b, coef = treatment_f4)
  ),
  chains = 4, cores = 4,
  file = "Stanfits/brm_m11_4"
)
print(summary(brm_m11.4))
```
...
post <- suppressWarnings(as_draws_df(brm_m11.4))
actor_cols <- paste0("b_actor_f", 1:7)
p_left <- inv_logit(as.matrix(post[, actor_cols]))
p_mean <- colMeans(p_left)
p_ci   <- apply(p_left, 2, PI)
plot(p_mean, 7:1, xlim = c(0, 1), pch = 1, yaxt = "n",
     ylab = "", xlab = "probability pull left")
axis(2, at = 7:1, labels = paste0("V", 1:7), las = 1)
for(i in 1:7) {
  abline(h = 8 - i, lty = 3, col = "gray")
  lines(p_ci[, i], c(8 - i, 8 - i))
}
```
...
# In brms 0+actor_f+treatment_f: treatment_f1 is reference (=0)
...
b_mean <- colMeans(b_treatment)
b_ci   <- apply(b_treatment, 2, PI)
plot(b_mean, 4:1, xlim = range(b_ci), pch = 1, yaxt = "n",
     ylab = "", xlab = "log-odds")
axis(2, at = 4:1, labels = labs, las = 1)
for(i in 1:4) {
  abline(h = 5 - i, lty = 3, col = "gray")
  lines(b_ci[, i], c(5 - i, 5 - i))
}
abline(v = 0)
```
...
# b[1]=0(ref), b[2]=treatment_f2, b[3]=treatment_f3, b[4]=treatment_f4
diffs <- list(
  db13 = 0 - post$b_treatment_f3, 
  db24 = post$b_treatment_f2 - post$b_treatment_f4
)
diff_mean <- sapply(diffs, mean)
diff_ci   <- sapply(diffs, PI)
plot(diff_mean, 2:1, xlim = range(diff_ci), pch = 1, yaxt = "n",
     ylab = "", xlab = "log-odds difference")
axis(2, at = 2:1, labels = names(diffs), las = 1)
for(i in 1:2) {
  abline(h = 3 - i, lty = 3, col = "gray")
  lines(diff_ci[, i], c(3 - i, 3 - i))
}
abline(v = 0)
```
...
newdat <- data.frame(
  actor_f     = factor(rep(1:7, each = 4), levels = levels(d$actor_f)),
  treatment_f = factor(rep(1:4, times = 7), levels = levels(d$treatment_f))
)
p_post <- posterior_epred(brm_m11.4, newdata = newdat)
...
```{r introBayes13-09b, cache=TRUE, results="hide", eval=FALSE}
# [LEGACY rethinking - preserved for reference]
m11.5 <- ulam(
  alist(
    pulled_left  ~ dbinom( 1, p ), 
...
brm_m11.5 <- brm(
  pulled_left ~ 0 + actor_f + side_f + cond_f,
  family = bernoulli(),
  data = d,
  prior = c(
    prior(normal(0, 1.5), class = b, coef = actor_f1),
    ...
    prior(normal(0, 1.5), class = b, coef = actor_f6),
    prior(normal(0, 1.5), class = b, coef = actor_f7),
    prior(normal(0, 0.5), class = b, coef = side_f2),
    prior(normal(0, 0.5), class = b, coef = cond_f2)
  ),
  chains = 4, cores = 4,
  file = "Stanfits/brm_m11_5"
)
print(summary(brm_m11.5))

brm_m11.4 <- add_criterion(brm_m11.4, "waic")
brm_m11.5 <- add_criterion(brm_m11.5, "waic")
print(loo_compare(brm_m11.4, brm_m11.5, criterion = "waic"))
```
...
```{r introBayes13-11, cache=TRUE, results="hide", eval=FALSE}
# [LEGACY rethinking - preserved for reference]
...
d_aggregated$actor_f     <- factor(d_aggregated$actor)
d_aggregated$treatment_f <- factor(d_aggregated$treatment)

brm_m11.6 <- brm(
  left_pulls | trials(18) ~ 0 + actor_f + treatment_f,
  family = binomial(),
  data = d_aggregated,
  prior = c(
    prior(normal(0, 1.5), class = b, coef = actor_f1),
  ...
  chains = 4, cores = 4,
  file = "Stanfits/brm_m11_6"
)
print(summary(brm_m11.6))
```
...
```{r  introBayes13-12, cache=TRUE, error=TRUE}
# loo_compare() will error because the two models have different observation
# counts (aggregated 28 vs individual 504).  This is exactly the point:
# model-comparison metrics are not comparable across different data formats.
brm_m11.6 <- add_criterion(brm_m11.6, "loo")
brm_m11.4 <- add_criterion(brm_m11.4, "loo")
tryCatch(
  print(loo_compare(brm_m11.6, brm_m11.4, criterion = "loo")),
  error = function(e) cat("Expected error:", conditionMessage(e), "\n")
)
# But parameter estimates are identical:
cat("\nbrm_m11.4 (individual):\n"); print(fixef(brm_m11.4))
cat("\nbrm_m11.6 (aggregated):\n"); print(fixef(brm_m11.6))
```
...
d$gid_f <- factor(ifelse(d$applicant.gender == "male", 1, 2))
```

```{r introBayes13-15b, cache=TRUE, results="hide", eval=FALSE}
# [LEGACY rethinking - preserved for reference]
m11.7 <- ulam(
  alist(
...
brm_m11.7 <- brm(
  ...
  data = d,
  prior = c(
    prior(normal(0, 1.5), class = b, coef = gid_f1),
    prior(normal(0, 1.5), class = b, coef = gid_f2)
  ),
  chains = 4, cores = 4,
  file = "Stanfits/brm_m11_7"
)
```

```{r introBayes13-1501-summary, cache=FALSE}
summary(brm_m11.7)
```
...
post <- suppressWarnings(as_draws_df(brm_m11.7))
diff_a <- post$b_gid_f1 - post$b_gid_f2
diff_p <- inv_logit(post$b_gid_f1) - inv_logit(post$b_gid_f2)
cat("diff_a: mean =", mean(diff_a), " PI =", PI(diff_a), "\n")
cat("diff_p: mean =", mean(diff_p), " PI =", PI(diff_p), "\n")
```
...
# Posterior predictive check (replaces postcheck)
pp <- posterior_epred(brm_m11.7)
pp_mean <- apply(pp, 2, mean)
pp_ci   <- apply(pp, 2, PI)
pp_sim  <- posterior_predict(brm_m11.7)
pp_sim_ci <- apply(pp_sim, 2, PI)

plot(1:nrow(d), d$admit / d$applications, pch = 16, col = rangi2,
     ylim = c(0, 1), xlab = "case", ylab = "admit",
     xaxt = "n")
axis(1, at = 1:nrow(d))
# Posterior mean & CI (proportion scale)
points(1:nrow(d), pp_mean / d$applications, pch = 1)
for(i in 1:nrow(d)) {
  lines(c(i, i), pp_ci[, i] / d$applications[i], lwd = 1)
  points(c(i, i), pp_sim_ci[, i] / d$applications[i], pch = 3, cex = 0.6)
}
...
d$dept_id_f <- factor(rep(1:6, each = 2))
```

```{r introBayes13-17b, cache=TRUE, results="hide", eval=FALSE}
# [LEGACY rethinking - preserved for reference]
m11.8 <- ulam(
  alist(
    admit ~ dbinom( applications, p ) , 
...
brm_m11.8 <- brm(
  admit | trials(applications) ~ 0 + gid_f + dept_id_f,
  family = binomial(),
  data = d,
  prior = c(
    prior(normal(0, 1.5), class = b, coef = gid_f1),
    prior(normal(0, 1.5), class = b, coef = gid_f2),
    prior(normal(0, 1.5), class = b, coef = dept_id_f2),
    prior(normal(0, 1.5), class = b, coef = dept_id_f3),
    prior(normal(0, 1.5), class = b, coef = dept_id_f4),
    prior(normal(0, 1.5), class = b, coef = dept_id_f5),
    prior(normal(0, 1.5), class = b, coef = dept_id_f6)
  ),
  chains = 4, cores = 4, iter = 4000,
  file = "Stanfits/brm_m11_8"
)
```

```{r introBayes13-1701-summary, cache=FALSE}
summary(brm_m11.8)
```
...
post <- suppressWarnings(as_draws_df(brm_m11.8))
diff_a <- post$b_gid_f1 - post$b_gid_f2
diff_p <- inv_logit(post$b_gid_f1) - inv_logit(post$b_gid_f2)
cat("diff_a: mean =", mean(diff_a), " PI =", PI(diff_a), "\n")
cat("diff_p: mean =", mean(diff_p), " PI =", PI(diff_p), "\n")
...
# Posterior predictive check (replaces postcheck)
pp <- posterior_epred(brm_m11.8)
pp_mean <- apply(pp, 2, mean)
pp_ci   <- apply(pp, 2, PI)
pp_sim  <- posterior_predict(brm_m11.8)
pp_sim_ci <- apply(pp_sim, 2, PI)
...
d$P <- as.numeric(scale( log(d$population) )) # standardize the log(population) into mean = 0, sd = 1
d$contact_id <- ifelse( d$contact == "high", 2, 1)
d$cid_f <- factor(d$contact_id)
```
...
```{r introBayes13-23, cache=TRUE, results="hide", eval=FALSE}
# [LEGACY rethinking - preserved for reference]
dat <- list(
  T = d$total_tools,
  P = d$P,
...
brm_m11.9 <- brm(
  ...
  chains = 4, cores = 4,
  file = "Stanfits/brm_m11_9"
)

# interaction model: a[cid] + b[cid]*P
brm_m11.10 <- brm(
  total_tools ~ 0 + cid_f + cid_f:P,
  family = poisson(),
  data = d,
  prior = c(
    prior(normal(3, 0.5), class = b, coef = cid_f1),
    prior(normal(3, 0.5), class = b, coef = cid_f2),
    prior(normal(0, 0.2), class = b, coef = "cid_f1:P"),
    prior(normal(0, 0.2), class = b, coef = "cid_f2:P")
  ),
  chains = 4, cores = 4,
  file = "Stanfits/brm_m11_10"
)
```

```{r introBayes13-24-summary, cache=FALSE}
summary(brm_m11.9)
summary(brm_m11.10)
```

```{r introBayes13-24-loo, cache=TRUE, dependson="introBayes13-24"}
brm_m11.9  <- add_criterion(brm_m11.9, "loo", moment_match = TRUE)
brm_m11.10 <- add_criterion(brm_m11.10, "loo", moment_match = TRUE)
print(loo_compare(brm_m11.9, brm_m11.10, criterion = "loo"))
```

在原始的 `rethinking` 套件中使用標準 PSIS-LOO 計算時，會看見關於 `Pareto k` 值過高的警報，提示數據中存在一些對模型結果影響較大的觀察值（高槓桿點）。在 `brms` 中，我們使用了 `moment_match = TRUE` 選項來修正這些高 Pareto k 值的觀察點，使得 LOO-CV 的估計更加可靠。儘管如此，我們仍然可以通過繪製 PSIS 圖來觀察哪些觀察值對模型影響最大。
...
loo_m11.10 <- loo(brm_m11.10)
k <- loo_m11.10$diagnostics$pareto_k
plot(d$P, d$total_tools, 
     xlab = "log population (std)", 
     ylab = "total tolls", 
     col = rangi2, 
      bty = "n", 
     pch = ifelse( d$contact_id == 1, 1, 16), 
     lwd = 2, 
     ylim = c(0, 75), 
     cex = 1 + normalize(k))
...
newdat1 <- data.frame(cid_f = factor(1, levels = levels(d$cid_f)), P = P_seq)
lambda <- posterior_epred(brm_m11.10, newdata = newdat1)
...
newdat2 <- data.frame(cid_f = factor(2, levels = levels(d$cid_f)), P = P_seq)
lambda <- posterior_epred(brm_m11.10, newdata = newdat2)
lmu <- apply( lambda, 2, mean)
lci <- apply( lambda, 2, PI)
...
plot( d$population, d$total_tools, 
      ...
      xlab = "population", 
      ylab = "total tools", 
      col = rangi2, pch = ifelse( d$contact_id == 1, 1, 16), lwd = 2, 
