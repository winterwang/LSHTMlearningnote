```{r session08-setup, include=FALSE}
...
d <- read.csv("../backupfiles/WaffleDivorce.csv", sep = ";")
...
```{r introBayes08-05-brm, cache=TRUE, results='hide', message=FALSE}
brm_m5.1 <- brm(
  D ~ A,
  data = d,
  prior = c(
  ...
  file = "Stanfits/brm_m5_1"
)
```

```{r introBayes08-05-brm-summary, cache=FALSE}
summary(brm_m5.1)
```
...
for( i in 1:n) {
  mu <- a_prior[i] + bA_prior[i] * c(-2, 2)
  lines(c(-2, 2), mu, col = col.alpha("black", 0.4))
}
```
...
mu <- posterior_epred(brm_m5.1, newdata = data.frame(A = A_seq))
...
```{r introBayes08-06-brm, cache=TRUE, results='hide', message=FALSE}
brm_m5.2 <- brm(
  D ~ M,
  data = d,
  prior = c(
  ...
  file = "Stanfits/brm_m5_2"
)
```

```{r introBayes08-06-brm-summary, cache=FALSE}
summary(brm_m5.2)
```
...
mu <- posterior_epred(brm_m5.2, newdata = data.frame(M = M_seq))
mu.mean <- apply( mu, 2, mean )
mu.PI <- apply(mu, 2, PI)
...
```{r introBayes08-09-brm, cache=TRUE, results='hide', message=FALSE}
brm_m5.3 <- brm(
  D ~ A + M,
  data = d,
  prior = c(
  ...
  file = "Stanfits/brm_m5_3"
)
```

```{r introBayes08-09-brm-summary, cache=FALSE}
summary(brm_m5.3)
```
...
# Manual coeftab-style comparison of bA and bM across models
f1 <- fixef(brm_m5.1); f2 <- fixef(brm_m5.2); f3 <- fixef(brm_m5.3)
d1 <- suppressWarnings(as_draws_df(brm_m5.1))
d2 <- suppressWarnings(as_draws_df(brm_m5.2))
d3 <- suppressWarnings(as_draws_df(brm_m5.3))

comp <- data.frame(
  model = c("brm_m5.3", "brm_m5.1", "brm_m5.3", "brm_m5.2"),
  par   = c("bA", "bA", "bM", "bM"),
  mean  = c(mean(d3$b_A), mean(d1$b_A), mean(d3$b_M), mean(d2$b_M)),
  lo    = c(quantile(d3$b_A, 0.055), quantile(d1$b_A, 0.055),
            quantile(d3$b_M, 0.055), quantile(d2$b_M, 0.055)),
  hi    = c(quantile(d3$b_A, 0.945), quantile(d1$b_A, 0.945),
            quantile(d3$b_M, 0.945), quantile(d2$b_M, 0.945))
)
y_pos <- nrow(comp):1
plot(comp$mean, y_pos, xlim = range(c(comp$lo, comp$hi, 0)),
     pch = 16, yaxt = "n", ylab = "",
     xlab = "Posterior with 89% credible intervals")
segments(comp$lo, y_pos, comp$hi, y_pos)
axis(2, at = y_pos, labels = paste(comp$model, comp$par), las = 1)
abline(v = 0, lty = 2)
text(comp$mean[1] - 0.15, y_pos[1] + 0.3, "bA", font = 2)
text(comp$mean[3] + 0.15, y_pos[3] + 0.3, "bM", font = 2)
```
...
```{r introBayes08-10-brm, cache=TRUE, results='hide', message=FALSE}
brm_m5.4 <- brm(
  M ~ A,
  data = d,
  prior = c(
  ...
  file = "Stanfits/brm_m5_4"
)
```

```{r introBayes08-10-brm-summary, cache=FALSE}
summary(brm_m5.4)
```
之後，我們計算結婚率的殘差的方法就是取觀測到的結婚率和上述模型給出的預測結婚率：

```{r introBayes08-11, cache=TRUE}
mu <- posterior_epred(brm_m5.4)
...
# Compute residual regression line and CI using simple lm
resid_lm <- lm(D ~ mu_res, data = d)
M_seq <- seq(from = -1.8, to = 1.8, length.out = 30)
pred <- predict(resid_lm, newdata = data.frame(mu_res = M_seq), interval = "confidence")

plot( D ~ mu_res, data = d, col = rangi2, 
       xlab = "Marriage rate residuals", 
     ylab = "Divorce rate (std, per/1000 adults)")
lines( M_seq, pred[, "fit"], lwd = 2)
shade( rbind(pred[, "lwr"], pred[, "upr"]), M_seq )
abline(v = 0, lty = 2)
```
...
brm_m5.41 <- brm(
  ...
  file = "Stanfits/brm_m5_41"
)

# calculate the predictor residual for A
mu <- posterior_epred(brm_m5.41)
mu_mean <- apply( mu, 2, mean )
mu_resid <- d$A - mu_mean

# Compute residual regression line and CI
d$muA_res <- mu_resid
resid_lm2 <- lm(D ~ muA_res, data = d)
M_seq <- seq(from = -1.8, to = 2.5, length.out = 30)
pred <- predict(resid_lm2, newdata = data.frame(muA_res = M_seq), interval = "confidence")
...
## Posterior predictive summary using brms
mu_samples <- posterior_epred(brm_m5.3)  # rows = draws, cols = cases
mu_mean <- apply(mu_samples, 2, mean)
mu_PI   <- apply(mu_samples, 2, PI, prob = 0.89)

# Posterior predictive simulations
D_sim <- posterior_predict(brm_m5.3)
...
In brms, the multivariate counterfactual model uses `bf()` + `set_rescor(FALSE)` to jointly fit the D and M equations. This is equivalent to the rethinking `quap()` approach above, which estimates both equations simultaneously:

```{r introBayes08-13-brm, cache=TRUE, results='hide', message=FALSE}
brm_m5.3_A <- brm(
  bf(D ~ A + M) + bf(M ~ A) + set_rescor(FALSE),
  data = d,
  prior = c(
  ...
  file = "Stanfits/brm_m5_3_A"
)
```

```{r introBayes08-13-brm-summary, cache=FALSE}
summary(brm_m5.3_A)
```
...
接下來不再使用過去的 `sim()` 便利函數，而是直接用 posterior samples 人工構造（manual）counterfactual 模擬：對每一個 A 的操縱值，先根據 posterior 抽樣得到對應的 M 分佈，再將該模擬出的 M 帶入 D 的線性預測式，生成 D 的模擬值。這樣的手動模擬與原書中 `sim()` 的邏輯等價，但更透明、也避免了因版本差異造成的函數失效。

```{r introBayes08-15, cache=TRUE}
## Counterfactual simulation using brms multivariate model draws
draws_cf <- suppressWarnings(as_draws_df(brm_m5.3_A))
n_draws <- nrow(draws_cf)

# Map brms column names to rethinking-style names for clarity:
# b_D_Intercept -> a, b_D_A -> bA, b_D_M -> bM, sigma_D -> sigma
# b_M_Intercept -> aM, b_M_A -> bAM, sigma_M -> sigma_M

# For each A value, compute M mean and simulate M, then compute D mean and simulate D.
sim_list_D <- vector("list", length(A_seq))
for(j in seq_along(A_seq)) {
  A_val <- A_seq[j]
  mu_M_j <- draws_cf$b_M_Intercept + draws_cf$b_M_A * A_val
  M_sim_j <- rnorm(n_draws, mu_M_j, draws_cf$sigma_M)
  mu_D_j <- draws_cf$b_D_Intercept + draws_cf$b_D_M * M_sim_j + draws_cf$b_D_A * A_val
  D_sim_j <- rnorm(n_draws, mu_D_j, draws_cf$sigma_D)
  sim_list_D[[j]] <- D_sim_j
}

# Combine into matrix: rows = draws, cols = A values
D_cf_mat <- do.call(cbind, sim_list_D)

# Summary intervals for D
D_cf_mean <- apply(D_cf_mat, 2, mean)
D_cf_PI   <- apply(D_cf_mat, 2, PI, prob = 0.89)

# Store for later plotting
cf_total <- list(A = A_seq, mean = D_cf_mean, PI = D_cf_PI)
```


繪製 simulated 數據的圖：

```{r introBayes08-fig11, cache=TRUE,  fig.width=5.5, fig.height=4.8,  fig.cap="Visualize the predicted effect of manipulating age at marriage A on divorce rate D. (Total causal effect of A on D, including A -> D and A -> M -> D pathways.)", fig.align='center'}
# Manual counterfactual plot using previously computed cf_total (D counterfactuals)
plot(cf_total$A, cf_total$mean, ylim = c(-2, 2), type = "l",
     xlab = "Manipulated A", ylab = "counterfactual D")
shade(cf_total$PI, cf_total$A)
mtext("Total counterfactual effect of A on D")
```



```{r introBayes08-fig12, cache=TRUE,  fig.width=5.5, fig.height=4.8,  fig.cap="Visualize the predicted effect of manipulating age at marriage A on marriage rate M (A -> M pathway).", fig.align='center'}
# Counterfactual simulations for M using brms draws
if(!exists("draws_cf")) draws_cf <- suppressWarnings(as_draws_df(brm_m5.3_A))
if(!exists("A_seq")) A_seq <- seq(from = -2, to = 2, length.out = 30)
n_draws <- nrow(draws_cf)
M_cf_mat <- sapply(A_seq, function(A_val){
  mu_M <- draws_cf$b_M_Intercept + draws_cf$b_M_A * A_val
  rnorm(n_draws, mu_M, draws_cf$sigma_M)
})
M_cf_mean <- colMeans(M_cf_mat)
M_cf_PI   <- apply(M_cf_mat, 2, PI, prob = 0.89)
plot(A_seq, M_cf_mean, ylim = c(-2, 2), type = "l",
...
# Effect of raising median age at marriage from 20 to 30 (standardized scale)
age_mean <- 26.1
age_sd   <- 1.24
A_vals_std <- (c(20,30) - age_mean) / age_sd
if(!exists("draws_cf")) draws_cf <- suppressWarnings(as_draws_df(brm_m5.3_A))
n_draws <- nrow(draws_cf)
# Simulate sequentially: A -> M then M,A -> D for both A values
simulate_D_for_A <- function(A_std){
  mu_M <- draws_cf$b_M_Intercept + draws_cf$b_M_A * A_std
  M_sim <- rnorm(n_draws, mu_M, draws_cf$sigma_M)
  mu_D <- draws_cf$b_D_Intercept + draws_cf$b_D_M * M_sim + draws_cf$b_D_A * A_std
  rnorm(n_draws, mu_D, draws_cf$sigma_D)
}
D_A20 <- simulate_D_for_A(A_vals_std[1])
D_A30 <- simulate_D_for_A(A_vals_std[2])
mean(D_A30 - D_A20)
```
...
# Manual counterfactual simulation: manipulate M while fixing A = 0
sim_dat <- data.frame( M = seq(from = -2 , to = 2, length.out = 30), A = 0)
if(!exists("draws_cf")) draws_cf <- suppressWarnings(as_draws_df(brm_m5.3_A))
n_draws <- nrow(draws_cf)
# For each manipulated M value, simulate D from posterior
D_M_mat <- sapply(sim_dat$M, function(M_val){
  mu_D <- draws_cf$b_D_Intercept + draws_cf$b_D_M * M_val + draws_cf$b_D_A * 0
  rnorm(n_draws, mu_D, draws_cf$sigma_D)
})
D_M_mean <- colMeans(D_M_mat)
D_M_PI   <- apply(D_M_mat, 2, PI, prob = 0.89)
plot(sim_dat$M, D_M_mean, ylim = c(-2, 2), type = "l",
  xlab = "manipulated M", ylab = "counterfactual D")
shade(D_M_PI, sim_dat$M)
...
d <- read.csv("../backupfiles/milk.csv", sep = ";")
str(d)
```
...
for( i in 1:n){
  mu <- a_prior[i] + bN_prior[i] * xseq
  lines( xseq, mu, col = col.alpha("black", 0.3))
}
```
...
```{r introBayes08-fig16-brm-fit, eval=FALSE}
# [LEGACY rethinking — preserved for reference]
# The quap model m5.5 with better priors:
m5.5 <- quap(
  alist(
    K ~ dnorm( mu, sigma ), 
    mu <- a + bN * N, 
...
```{r introBayes08-fig16-brm, cache=TRUE, results='hide', message=FALSE}
brm_m5.5 <- brm(
  K ~ N,
  data = dcc,
  prior = c(
...
mu <- posterior_epred( brm_m5.5, newdata = data.frame(N = xseq))
mu_mean <- apply(mu, 2, mean)
mu_PI <- apply(mu, 2, PI)
...
```{r introBayes08-24-brm, cache=TRUE, results='hide', message=FALSE}
brm_m5.6 <- brm(
  K ~ M,
  data = dcc,
  prior = c(
  ...
  file = "Stanfits/brm_m5_6"
)
```

```{r introBayes08-24-brm-summary, cache=FALSE}
summary(brm_m5.6)
```
...
mu <- posterior_epred( brm_m5.6, newdata = data.frame(M = xseq))
mu_mean <- apply(mu, 2, mean)
mu_PI <- apply(mu, 2, PI)
...
```{r introBayes08-25-brm, cache=TRUE, results='hide', message=FALSE}
brm_m5.7 <- brm(
  K ~ N + M,
  data = dcc,
  prior = c(
  ...
  file = "Stanfits/brm_m5_7"
)
```

```{r introBayes08-25-brm-summary, cache=FALSE}
summary(brm_m5.7)
```
...
# Manual coeftab-style comparison of bM and bN across models
d5 <- suppressWarnings(as_draws_df(brm_m5.5))
d6 <- suppressWarnings(as_draws_df(brm_m5.6))
d7 <- suppressWarnings(as_draws_df(brm_m5.7))

comp <- data.frame(
  model = c("brm_m5.7", "brm_m5.6", "brm_m5.7", "brm_m5.5"),
  par   = c("bM", "bM", "bN", "bN"),
  mean  = c(mean(d7$b_M), mean(d6$b_M), mean(d7$b_N), mean(d5$b_N)),
  lo    = c(quantile(d7$b_M, 0.055), quantile(d6$b_M, 0.055),
            quantile(d7$b_N, 0.055), quantile(d5$b_N, 0.055)),
  hi    = c(quantile(d7$b_M, 0.945), quantile(d6$b_M, 0.945),
            quantile(d7$b_N, 0.945), quantile(d5$b_N, 0.945))
)
y_pos <- nrow(comp):1
plot(comp$mean, y_pos, xlim = range(c(comp$lo, comp$hi, 0)),
     pch = 16, yaxt = "n", ylab = "",
...
axis(2, at = y_pos, labels = paste(comp$model, comp$par), las = 1)
abline(v = 0, lty = 2)
text(comp$mean[1] - 0.1, y_pos[1] + 0.3, "bM", font = 2)
text(comp$mean[3] + 0.1, y_pos[3] + 0.3, "bN", font = 2)
```
...
mu <- posterior_epred(brm_m5.7, newdata = data.frame( N = xseq, M = 0 ))
...
mu <- posterior_epred(brm_m5.7, newdata = data.frame( M = xseq, N = 0 ))
...
d <- read.csv("../backupfiles/Howell1.csv", sep = ";")
str(d)
```
...
df_prior <- data.frame(mu_female, mu_male)
data.frame(
  mean = colMeans(df_prior),
  sd = apply(df_prior, 2, sd),
  `5.5%` = apply(df_prior, 2, quantile, 0.055),
  `94.5%` = apply(df_prior, 2, quantile, 0.945),
  check.names = FALSE
)
```
...
```{r introBayes08-29-brm, cache=TRUE, results='hide', message=FALSE}
...
brm_m5.8 <- brm(
  ...
  file = "Stanfits/brm_m5_8"
)
```

```{r introBayes08-29-brm-summary, cache=FALSE}
summary(brm_m5.8)
```
...
post <- suppressWarnings(as_draws_df(brm_m5.8))
post$diff_fm <- post$b_sex_f1 - post$b_sex_f2
cat("Female - Male height difference:\n")
cat("Mean:", mean(post$diff_fm), "\n")
cat("89% CI:", quantile(post$diff_fm, 0.055), "to", quantile(post$diff_fm, 0.945), "\n")
```
...
```{r introBayes08-33-brm, cache=TRUE, results='hide', message=FALSE}
d$K <- standardize( d$kcal.per.g )
d$clade_f <- factor(d$clade_id)
brm_m5.9 <- brm(
  K ~ 0 + clade_f,
  data = d,
  prior = c(
  ...
  file = "Stanfits/brm_m5_9"
)
```

```{r introBayes08-33-brm-summary, cache=FALSE}
summary(brm_m5.9)
```
...
fe <- fixef(brm_m5.9)
labels <- paste(rownames(fe), ":", levels(d$clade), sep = "")
y_pos <- nrow(fe):1
plot(fe[, "Estimate"], y_pos,
     xlim = range(fe[, c("Q2.5", "Q97.5")]),
     pch = 16, yaxt = "n", ylab = "",
     xlab = "expected kcal (std)")
segments(fe[, "Q2.5"], y_pos, fe[, "Q97.5"], y_pos)
