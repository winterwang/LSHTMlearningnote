...
```{r introBayes12-05, cache=TRUE, results="hide", eval=FALSE}
m9.1 <- ulam(
  alist(
    log_gdp_std ~ dnorm( mu, sigma ), 
    mu <- a[cid] + b[cid] * ( rugged_std - 0.215 ) , 
...
```{r introBayes12-06, cache=TRUE, eval=FALSE}
m9.104 <- ulam(
  alist(
    log_gdp_std ~ dnorm( mu, sigma ), 
    mu <- a[cid] + b[cid] * ( rugged_std - 0.215 ) , 
...
```{r introBayes12-08, cache=TRUE, results="hide", eval=FALSE}
y <- c(-1, 1)
set.seed(11)
m9.2 <- ulam(
  alist(
...
```{r  introBayes12-09, cache=TRUE, results="hide", eval=FALSE}
set.seed(11)
m9.3 <- ulam(
  alist(
    y ~ dnorm( mu, sigma ), 
...
```{r introBayes12-11, cache=TRUE, results="hide", eval=FALSE}
set.seed(384)
m9.4 <- ulam(
  alist(
    y ~ dnorm( mu, sigma ),
...
```{r introBayes12-12, cache=TRUE, results="hide", eval=FALSE}
set.seed(384)
m9.5 <- ulam(
  alist(
    y ~ dnorm( mu, sigma ),
