...
```{r introBayes12-05, cache=TRUE, results="hide"}
m9.1 <- ulam(
  alist(
    log_gdp_std ~ dnorm( mu, sigma ), 
    mu <- a[cid] + b[cid] * ( rugged_std - 0.215 ) , 
...
```{r introBayes12-08, cache=TRUE, results="hide"}
y <- c(-1, 1)
set.seed(11)
m9.2 <- ulam(
  alist(
...
```{r  introBayes12-09, cache=TRUE, results="hide"}
set.seed(11)
m9.3 <- ulam(
  alist(
    y ~ dnorm( mu, sigma ), 
...
```{r introBayes12-11, cache=TRUE, results="hide"}
set.seed(384)
m9.4 <- ulam(
  alist(
    y ~ dnorm( mu, sigma ),
...
```{r introBayes12-12, cache=TRUE, results="hide"}
set.seed(384)
m9.5 <- ulam(
  alist(
    y ~ dnorm( mu, sigma ),
