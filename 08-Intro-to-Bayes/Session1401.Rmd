```{r introBayes14-00-setup, cache=FALSE, include=FALSE}
...
library(gtools)       # rdirichlet for Dirichlet prior visualisation
```
...
```{r introBayes14-01-brm, cache=TRUE, results="hide", message=FALSE}
d <- read.csv("../backupfiles/UCBadmit.csv", sep = ";")
d$gid   <- ifelse(d$applicant.gender == "male", 1L, 2L)
d$gid_f <- factor(d$gid)

b12.1 <- brm(
  admit | trials(applications) ~ 0 + gid_f,
  family  = beta_binomial(),
  prior   = c(prior(normal(0, 1.5), class = b),
              prior(exponential(1), class = phi)),
  data    = d,
  chains  = 4, cores = 4,
  file    = "Stanfits/brm_m12_1"
)
```


在 rethinking 中，我們爲 `theta` 使用了 `transpars>` 標記使 Stan 保存轉換參數。在 brms 的 `beta_binomial()` 家族中，分散程度參數被直接命名爲 `phi` 並自動保存在採樣結果中（注意這裏的 `phi` 和原模型中的 $\phi$ 不完全相同——brms 的 `phi` 直接對應原模型的 $\theta = \phi + 2$，即 brms 內部不需要加2的變換）。我們可以使用採樣過後的 `b12.1` 計算其事後的男女錄取率之差:
 

```{r introBayes14-02, cache=FALSE}
summary(b12.1)

draws <- as_draws_df(b12.1)
draws$da <- draws$b_gid_f1 - draws$b_gid_f2
cat("\nDifference a[1]-a[2] (log-odds):\n")
print(posterior::summarise_draws(
  posterior::as_draws_df(data.frame(
    a1 = draws$b_gid_f1, a2 = draws$b_gid_f2,
    da = draws$da, phi = draws$phi)),
  mean, sd, ~quantile(.x, probs = c(0.055, 0.945))))
```
...
```{r introBayes14-fig02, cache=TRUE, fig.width=6, fig.height=5,  fig.cap="Posterior distributions for b12.1. The thick curves are the posterior mean beta distribution for male and female applicants. The ligher curves represents 50 combinations of bar(p) and phi sampled from the posterior.", fig.align='center'}
draws <- as_draws_df(b12.1)
gid <- 2

# draw posterior mean beta distribution
curve( dbeta2(x, mean(inv_logit(draws$b_gid_f2)), mean(draws$phi)), 
       from = 0, to = 1, ylab = "Density", xlab = "probability admit", 
       ylim = c(0, 3), lwd = 2, bty = "n")
curve( dbeta2(x, mean(inv_logit(draws$b_gid_f1)), mean(draws$phi)), 
       from = 0, to = 1, ylab = "Density", xlab = "probability admit", 
       ylim = c(0, 3), lwd = 2, add =  TRUE, col = rangi2)
# Draw 50 beta distributions sampled from posterior

for( i in 1:50 ) {
  p <- inv_logit( draws$b_gid_f2[i] )
  phi_val <- draws$phi[i]
  curve( dbeta2(x, p, phi_val), add = TRUE, col = col.alpha("black", 0.2))
}
for( i in 1:50 ) {
  p <- inv_logit( draws$b_gid_f1[i] )
  phi_val <- draws$phi[i]
  curve( dbeta2(x, p, phi_val), add = TRUE, col = col.alpha(rangi2, 0.2))
}
...
```{r introBayes14-fig03, cache=TRUE, fig.width=6, fig.height=5,  fig.cap="Posterior validation check for b12.1. As a result of widely dispersed beta distributions on the figure above, the raw data (blue) is contained within the prediction intervals. Note: unlike the rethinking postcheck() which plots admission rates (proportions 0–1 on the y-axis), brms posterior_predict() generates counts on the original scale, so the y-axis here shows the number of admissions.", fig.align='center'}
# Posterior predictive check — replaces rethinking::postcheck()
# Note: brms posterior_predict() returns counts (not proportions),
# so the scale differs from the original postcheck() figure.
yrep <- posterior_predict(b12.1)
pred_mean <- colMeans(yrep)
pred_ci   <- apply(yrep, 2, PI)

plot(1:nrow(d), d$admit, pch = 16, col = rangi2,
     ylim = range(c(d$admit, pred_ci)),
     xlab = "case", ylab = "admit", bty = "n",
     main = "Posterior predictive check (b12.1)")
points(1:nrow(d), pred_mean, pch = 1)
for (i in 1:nrow(d)) lines(c(i, i), pred_ci[, i])
```
...
```{r introBayes14-03-brm, cache=TRUE, results="hide", message=FALSE}
d <- read.csv("../backupfiles/Kline.csv", sep = ";")
d$cid   <- ifelse(d$contact == "high", 2L, 1L)
d$cid_f <- factor(d$cid)
d$logP  <- log(d$population)

# Nonlinear formula: log(mu) = a + b*logP  ↔  mu = exp(a)*P^b
# (g from original model absorbed into per-cid intercepts a)
b12.2 <- brm(
  bf(total_tools ~ a + b * logP,
     a ~ 0 + cid_f,
     b ~ 0 + cid_f,
     nl = TRUE),
  family = negbinomial(),
  ...
  data   = d,
  chains = 4, cores = 4,
  file   = "Stanfits/brm_m12_2"
)
```


```{r introBayes14-04, cache=FALSE}
summary(b12.2)
```
...
b12.2 <- add_criterion(b12.2, "loo", moment_match = TRUE)
k <- b12.2$criteria$loo$diagnostics$pareto_k
...
newdata_low <- data.frame(logP = log(pop_seq),
                          cid_f = factor(1, levels = levels(d$cid_f)))
lambda <- posterior_epred(b12.2, newdata = newdata_low)
...
newdata_high <- data.frame(logP = log(pop_seq),
                           cid_f = factor(2, levels = levels(d$cid_f)))
lambda <- posterior_epred(b12.2, newdata = newdata_high)
lmu <- apply( lambda, 2, mean )
lci <- apply( lambda, 2, PI )
...
```{r introBayes14-06-brm, cache=TRUE, results="hide", message=FALSE}
b12.3 <- brm(
  bf(y ~ 1, zi ~ 1),
  ...
  data   = data.frame(y = y),
  chains = 4, cores = 4,
  file   = "Stanfits/brm_m12_3"
)
```

```{r introBayes14-07, cache=FALSE}
summary(b12.3)
```

可以計算模型中估計的這些工人偷懶的日子所佔的比例是多少：

```{r introBayes14-08, cache=FALSE}
draws <- as_draws_df(b12.3)
mean( inv_logit( draws$b_zi_Intercept )) # probability of drink/not working days
mean( exp( draws$b_Intercept ))          # rate finish manuscripts, when working
```
...
d <- read.csv("../backupfiles/Trolley.csv", sep = ";")
...
d_plot <-
  ...
  mutate(pr_k     = n / total_n,
         cum_pr_k = cumsum(pr_k)) %>% 
  mutate(discrete_probability = if_else(response == 1, cum_pr_k, cum_pr_k - pr_k))
...
```{r introBayes14-0601-brm, cache=TRUE, results="hide", message=FALSE}
d$response_f <- ordered(d$response)
...
b12.4 <- brm(
  ...
  data   = d,
  chains = 4, cores = 4,
  file   = "Stanfits/brm_m12_4"
)
```


```{r introBayes14-0701, cache=FALSE}
summary(b12.4)
```

可以看見每個截距的事後估計量都被計算得十分精確。我們可以使用邏輯函數的逆函數把累積概率簡單地計算回來：

```{r introBayes14-0801, cache=FALSE}
# Extract threshold estimates (cutpoints)
thresholds <- fixef(b12.4)[, "Estimate"]
round( inv_logit(thresholds), 3)
```
...
```{r  introBayes14-09, cache=FALSE}
round( pk <- dordlogit(1:7, 0, thresholds), 2 )
```
...
```{r  introBayes14-11, cache=FALSE}
round( pk <- dordlogit(1:7, 0, thresholds - 0.5 ), 2 )
```
...
```{r introBayes14-13-brm, cache=TRUE, results="hide", message=FALSE}
b12.5 <- brm(
  response_f ~ action + contact + intention + action:intention + contact:intention,
  family = cumulative("logit"),
  prior  = c(prior(normal(0, 0.5), class = b),
             prior(normal(0, 1.5), class = Intercept)),
  data   = d,
  chains = 4, cores = 4,
  file   = "Stanfits/brm_m12_5"
)
```



```{r  introBayes14-14, cache=FALSE}
summary(b12.5)
```
...
```{r introBayes14-fig10, cache=TRUE, fig.width=7, fig.height=5,  fig.cap="The marginal posterior distributions of the slopes in b12.5.", fig.align='center'}
# precis-style caterpillar plot for slope parameters
fe <- fixef(b12.5)
slope_idx <- !grepl("Intercept", rownames(fe))
fe_s <- fe[slope_idx, , drop = FALSE]

par(mar = c(5, 10, 2, 2))  # wider left margin for labels
plot(fe_s[, "Estimate"], seq_len(nrow(fe_s)),
     xlim = c(-1.4, 0), pch = 1, cex = 1.2,
     xlab = "Value", ylab = "", yaxt = "n", bty = "n")
segments(fe_s[, "Q2.5"],  seq_len(nrow(fe_s)),
         fe_s[, "Q97.5"], seq_len(nrow(fe_s)))
axis(2, at = seq_len(nrow(fe_s)), labels = rownames(fe_s), las = 1, cex.axis = 0.9)
abline(v = 0, lty = 3, col = "gray")
for (h in seq_len(nrow(fe_s))) abline(h = h, lty = 3, col = "gray")
...
```{r introBayes14-fig11, cache=TRUE, fig.width=6, fig.height=5,  fig.cap="Posterior predictions of the ordered categorical model with interactions, b12.5. The distribution of posterior probabilities of each outcome across values of intention for action = 0, and contact = 0.", fig.align='center'}
plot( NULL, type = "n", 
      bty = "n",
      xlab = "intention", 
      ylab = "probability", 
...
pdat <- data.frame(action = kA, contact = kC, intention = kI)

# Get linear predictor draws (phi)
phi <- posterior_linpred(b12.5, newdata = pdat)

# Get threshold draws
draws <- as_draws_df(b12.5)
thr_cols <- grep("^b_Intercept\\[", names(draws), value = TRUE)
thr_mat  <- as.matrix(draws[, thr_cols])

for( s in 1:50 ) {
  pk <- pordlogit( 1:6, phi[s, ], thr_mat[s, ])
...
```{r introBayes14-fig12, cache=TRUE, fig.width=6, fig.height=5,  fig.cap="Posterior predictions of the ordered categorical model with interactions, b12.5. The distribution of posterior probabilities of each outcome across values of intention for action = 1, and contact = 0.", fig.align='center', echo=FALSE}
plot( NULL, type = "n", 
      bty = "n",
      xlab = "intention", 
      ylab = "probability", 
...
```{r introBayes14-fig13, cache=TRUE, fig.width=6, fig.height=5,  fig.cap="Posterior predictions of the ordered categorical model with interactions, b12.5. The distribution of posterior probabilities of each outcome across values of intention for action = 0, and contact = 1.", fig.align='center', echo=FALSE}
plot( NULL, type = "n", 
      bty = "n",
      xlab = "intention", 
      ylab = "probability", 
...
```{r  introBayes14-fig14, cache=TRUE, fig.width=6, fig.height=5,  fig.cap="The same interactions in the b12.5 model visualized as histograms of simulated outcomes. The black line segments are intention =0. The blue line segments are intention = 1. ", fig.align='center'}
kA <- 0         # value of action
kC <- 0         # value for contact
kI <- 0:1       # values for intention to calculate over

pdat <- data.frame(action = kA, contact = kC, intention = kI)
s <- posterior_predict(b12.5, newdata = pdat)
# Overlay two histograms: intention=0 (black) and intention=1 (blue)
tab0 <- table(factor(s[,1], levels = 1:7))
tab1 <- table(factor(s[,2], levels = 1:7))
ymax <- max(c(tab0, tab1)) * 1.1
plot(1:7, tab0, type = "h", lwd = 4, col = "black",
     xlab = "response", ylab = "Frequency", bty = "n",
     xlim = c(0.5, 7.5), ylim = c(0, ymax),
     main = "action = 0, contact = 0")
lines(1:7 + 0.1, tab1, type = "h", lwd = 4, col = rangi2)
```

```{r  introBayes14-fig15, cache=TRUE, fig.width=6, fig.height=5,  fig.cap="The same interactions in the b12.5 model visualized as histograms of simulated outcomes. The black line segments are intention =0. The blue line segments are intention = 1. ", fig.align='center', echo=FALSE}
...
```{r  introBayes14-fig16, cache=TRUE, fig.width=6, fig.height=5,  fig.cap="The same interactions in the b12.5 model visualized as histograms of simulated outcomes. The black line segments are intention =0. The blue line segments are intention = 1. ", fig.align='center', echo=FALSE}
kA <- 0         # value of action
...
# Reload Trolley data (same CSV as above)
d <- read.csv("../backupfiles/Trolley.csv", sep = ";")
...
```{r introBayes14-16-brm, cache=TRUE, results="hide", message=FALSE}
# brms equivalent: mo() implements the same Dirichlet monotonic decomposition
d$edu_int <- as.integer(d$edu)
d$response_f <- ordered(d$response)

b12.6 <- brm(
  response_f ~ mo(edu_int) + action + intention + contact,
  family = cumulative("logit"),
  prior  = c(prior(normal(0, 1),   class = b),
             prior(normal(0, 1.5), class = Intercept),
             prior(dirichlet(2), class = simo, coef = "moedu_int1")),
  data   = d,
  chains = 4, cores = 4,
  file   = "Stanfits/brm_m12_6"
)
```
...
```{r introBayes14-17, cache=FALSE}
summary(b12.6)
```

受教育程度這個變量對結果的選擇的影響整體來說是負的 `moedu_int = -0.05`。也就是說，受教育程度最高的受試者，會傾向於不去拉動變軌的機制。然後我們來看這一系列的 `delta` 變量的運算結果（在 brms 中對應 `simo_moedu_int1` 系列參數）。圖 \@ref(fig:introBayes14-fig18) 顯示，每個學歷的增加都多多少少地增加了學歷對結果的影響，但是其中第五層級的學歷 `SCol = Some college` 比起從高中畢業 `HSG = high school graduate` 的增加可以說微不足道。
...
```{r introBayes14-18-brm, cache=TRUE, results="hide", message=FALSE}
d$edu_norm <- normalize(as.integer(d$edu))

b12.7 <- brm(
  response_f ~ edu_norm + action + intention + contact,
  family = cumulative("logit"),
  prior  = c(prior(normal(0, 1),   class = b),
             prior(normal(0, 1.5), class = Intercept)),
  data   = d,
  chains = 4, cores = 4,
  file   = "Stanfits/brm_m12_7"
)
```

```{r  introBayes14-19, cache=FALSE}
summary(b12.7)
```


你會看見，這時候給出的運算結果提示教育程度並不影響結果的選擇，或者影響幾乎可以忽略不計 `edu_norm = -0.12, 95% CrI: -0.30, 0.07` 。但是很顯然這不是事實，因爲不同教育程度之間的跨度，不能認爲是一個線性的變化過程，不同的教育程度的提升之間對結果的影響並不是簡單的一成不變的 different levels have different incremental associations。
