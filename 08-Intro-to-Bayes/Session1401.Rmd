...
```{r introBayes14-fig03, cache=TRUE, fig.width=6, fig.height=5,  fig.cap="Posterior validation check for b12.1. As a result of widely dispersed beta distributions on the figure above, the raw data (blue) is contained within the prediction intervals.", fig.align='center'}
...
```{r introBayes14-fig10, cache=TRUE, fig.width=6, fig.height=5,  fig.cap="The marginal posterior distributions of the slopes in b12.5.", fig.align='center'}
...
axis(2, at = seq_len(nrow(fe_s)), labels = rownames(fe_s), las = 1, cex.axis = 0.8)
...
你會看見，這時候給出的運算結果提示教育程度並不影響結果的選擇，或者影響幾乎可以忽略不計 `bE = -0.10, 89% CrI: -0.25, 0.05` 。但是很顯然這不是事實，因爲不同教育程度之間的跨度，不能認爲是一個線性的變化過程，不同的教育程度的提升之間對結果的影響並不是簡單的一成不變的 different levels have different incremental associations。
