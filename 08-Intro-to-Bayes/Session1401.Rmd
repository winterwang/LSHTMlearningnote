...
```{r  introBayes14-fig04, cache=TRUE, fig.width=6, fig.height=5,  fig.cap="", fig.align='center'}
...
lambda <- link( m12.2, data = data.frame( P = P_seq, 
                                          cid = 1))
...
lambda <- link( m12.2, data = data.frame( P = P_seq, cid = 2 ))
lmu <- apply( lambda, 2, mean )
lci <- apply( lambda, 2, PI )
