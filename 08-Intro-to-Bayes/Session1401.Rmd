...
```{r introBayes14-16, cache=TRUE, results="hide"}
dat <- list(
  R = d$response, 
  action = d$action,
...
```{r  introBayes14-fig18, cache=TRUE, fig.width=6, fig.height=5,  fig.cap="", fig.align='center'}
...
```{r introBayes14-18, cache=TRUE, results="hide"}
...
你會看見，這時候給出的運算結果提示教育程度並不影響結果的選擇，或者影響幾乎可以忽略不計 `bE = -0.10, 89% CrI: -0.25, 0.04` 。但是很顯然這不是事實，因爲不同教育程度之間的跨度，不能認爲是一個線性的變化過程，不同的教育程度的提升之間對結果的影響並不是簡單的一成不變的 different levels have different incremental associations。
