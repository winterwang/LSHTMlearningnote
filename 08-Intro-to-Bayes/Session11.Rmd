...
本章的代碼已從 `rethinking` 套件遷移至 [`brms`](https://paul-buerkner.github.io/brms/)。遷移後我們使用 `brms::brm()` 取代 `rethinking::quap()` / `ulam()` 來擬合貝葉斯模型，並以 `posterior_epred()` 取代 `link()`、`as_draws_df()` 取代 `extract.samples()` 等。爲了保持與原書繪圖風格的一致性，下方的設定程式碼定義了幾個輔助函數（`dens`、`PI`、`shade`、`col.alpha`、`concat`），它們模擬了 `rethinking` 套件中同名函數的行爲，使得後續的繪圖代碼可以幾乎不做修改地繼續使用。

此外，由於 macOS 系統升級後 SDK 路徑可能發生變化，導致 Stan 模型的 C++ 編譯失敗，因此需要透過 `Sys.setenv(SDKROOT = ...)` 明確指定正確的 SDK 路徑。`dir.create("Stanfits")` 則用於建立本地快取目錄——每個 `brm()` 模型擬合後都會以 `.rds` 檔案儲存於此目錄中，避免重複編譯，大幅縮短後續渲染時間。

```{r introBayes11-00, cache=FALSE, include=FALSE}
...
rugged <- read.csv("../backupfiles/rugged.csv", sep = ";")
d <- rugged
...
# center rugged_std for brms models
...
把這些翻譯成R語言模型（原始 rethinking 代碼保留供參考）：


```{r introBayes11-02, cache=TRUE, eval=FALSE}
...
set.seed(7)
N_samp <- 1000
prior_a_vague <- rnorm(N_samp, 1, 1)
prior_b_vague <- rnorm(N_samp, 0, 1)
...
with(dd, abline(a = 1.28, b = -0.6, lty = 1, lwd = 1.5, col = "blue"))
for( i in 1:50 ) lines( rugged_seq, 
                         prior_a_vague[i] + prior_b_vague[i] * (rugged_seq - 0.215), 
...
sum( abs(prior_b_vague) > 0.6 ) / length(prior_b_vague)
```
...
# Fit brms m8.1 with improved priors (cached to disk)
brm_m8.1 <- brm(
  ...
  family  = gaussian(),
  file    = "Stanfits/m8.1",
  chains  = 4, cores = 4, seed = 7,
  silent  = 2, refresh = 0
)

set.seed(7)
prior_a_better <- rnorm(1000, 1, 0.1)
prior_b_better <- rnorm(1000, 0, 0.3)
...
with(dd, abline(a = 1.28, b = -0.6, lty = 1, lwd = 1.5, col = "blue"))
for( i in 1:50 ) lines( rugged_seq, 
                         prior_a_better[i] + prior_b_better[i] * (rugged_seq - 0.215), 
                         col = col.alpha("black", 0.3))
```
...
dd$cid_f <- factor(dd$cid)  # factor for brms index variable
```
...
# brms: 0 + cid_f gives separate intercepts per level (Africa/non-Africa)
brm_m8.2 <- brm(
  log_gdp_std ~ 0 + cid_f + rugged_std_c,
  data    = dd,
  prior   = c(
...
print(summary(brm_m8.2))

# Model comparison: equivalent to compare(m8.1, m8.2)
brm_m8.1 <- add_criterion(brm_m8.1, "waic")
brm_m8.2 <- add_criterion(brm_m8.2, "waic")
print(loo_compare(brm_m8.1, brm_m8.2, criterion = "waic"))
```

可以看到，`cid_f1` 是非洲大陸國家的事後截距均值。可以看到它明顯是低於非洲大陸意外國家的事後截距均值 `cid_f2`。對兩者的事後比較估算可以給我們更加確定的結果：

```{r introBayes11-07, cache=TRUE}
post <- suppressWarnings(as_draws_df(brm_m8.2))
diff_a1_a2 <- post$b_cid_f1 - post$b_cid_f2
PI(diff_a1_a2)
```
...
mu.NotAfrica <- posterior_epred(
  brm_m8.2, 
  newdata = data.frame(cid_f = factor(2, levels = c(1,2)), 
                       rugged_std_c = rugged.seq - 0.215))
mu.Africa <- posterior_epred(
  brm_m8.2, 
  newdata = data.frame(cid_f = factor(1, levels = c(1,2)), 
                       rugged_std_c = rugged.seq - 0.215))
...
with(dd, points(rugged_std[cid == 1], log_gdp_std[cid == 1], col = rangi2, pch = 16))
...
# brms: separate intercepts and slopes per cid level
brm_m8.3 <- brm(
...
brm_m8.3 <- add_criterion(brm_m8.3, "waic")
print(loo_compare(brm_m8.1, brm_m8.2, brm_m8.3, criterion = "waic"))
```
...
plot(d.A1$rugged_std, d.A1$log_gdp_std, pch = 16, 
     col = rangi2, bty = "n", main = "African nations",
     xlab = "ruggedness (standardized)", 
     ylab = "log GDP (as proportion of mean)", xlim = c(0,1))
mu_A1 <- posterior_epred(
  brm_m8.3, 
  newdata = data.frame(cid_f = factor(1, levels = c(1,2)), 
                       rugged_std_c = rugged_seq - 0.215))
mu_mean_A1 <- apply( mu_A1, 2, mean )
mu_ci_A1   <- apply( mu_A1, 2, PI, prob = 0.97)
lines( rugged_seq, mu_mean_A1, lwd = 2)
shade(mu_ci_A1, rugged_seq, col = col.alpha(rangi2, 0.3))
```
...
plot(d.A2$rugged_std, d.A2$log_gdp_std, pch = 1, 
     col = "black", bty = "n", main = "Non-African nations",
     xlab = "ruggedness (standardized)", 
     ylab = "log GDP (as proportion of mean)", xlim = c(0,1))
mu_A2 <- posterior_epred(
  brm_m8.3, 
  newdata = data.frame(cid_f = factor(2, levels = c(1,2)), 
                       rugged_std_c = rugged_seq - 0.215))
mu_mean_A2 <- apply( mu_A2, 2, mean )
mu_ci_A2   <- apply( mu_A2, 2, PI, prob = 0.97)
lines( rugged_seq, mu_mean_A2, lwd = 2)
shade(mu_ci_A2, rugged_seq)
```
...
tulips <- read.csv("../backupfiles/tulips.csv", sep = ";")
d <- tulips
str(d)
```
...
summary(d[, c("blooms", "water", "shade", "blooms_std", "water_cent", "shade_cent")])
```
...
for (s in -1:1) {
  ...
  plot( d$water_cent[idx], d$blooms_std[idx], 
        xlim = c(-1, 1), bty = "n", ylim = c(0, 1), 
        main = paste("m8.4 (no interaction)\npost: shade = ", s),
        xlab = "water (centered)", ylab = "blooms (rescaled)", 
        pch = 16, col = rangi2)
  mu <- posterior_epred(brm_m8.4, 
                        newdata = data.frame(shade_cent = s, water_cent = -1:1))
  for( i in 1:20 ) lines( -1:1, mu[i, ], col = col.alpha("black", 0.3))
}
```

圖 \@ref(fig:introBayes11-fig06) 顯示的是沒有交互作用項時給出的事後預測直線。每個陽光，水條件下，水和開花尺寸之間關係的直線並沒有斜率上的顯著變化。給 `brm_m8.5` 繪製相似的三聯圖，可以看出斜率隨着不同陽光強度發生了變化：
