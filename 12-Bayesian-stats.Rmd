...
那麼我們無法獲得解析解 (analytical solution/closed solution) 的情況下，只好求助於數學上的數值解 (numerical solution/intergration)。
...
3. 然後:<br>從$p(\theta_1^{(1)}|\theta_2^{(0)},\theta_3^{(0)},\dots,\theta_k^{(0)},x)$中採集 $\theta_1^{(1)}$<br>從$p(\theta_2^{(1)}|\theta_1^{(1)},\theta_3^{(0)},\dots,\theta_k^{(0)},x)$中採集$\theta_2^{(1)}$<br>$\vdots$<br>從$p(\theta_k^{(1)}|\theta_1^{(1)},\theta_2^{(1)},\dots,\theta_{k-1}^{(0)},x)$中採集$\theta_k^{(1)}$
