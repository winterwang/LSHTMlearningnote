...
綜上所述，我們可以發現，雖然多層回歸模型可能可以克服個體之間存在依賴性的問題，但是模型中的參數需要滿足這樣的三個模型前提條件：

- （每次使用相同模型時，對應的）參數是相同的 (identical parameters)；
- （每個模型內，各個）參數是獨立的 (independent parameters)；
- （每個模型內，各個）參數是可置換 (exchangeable parameters)。
