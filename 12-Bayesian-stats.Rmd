...
The Analytic Techique lectures were orgainised and taught by Professor [Alexina Mason](https://www.lshtm.ac.uk/aboutus/people/mason.alexina) and supported by Dr. [F. Javier Rubio](https://sites.google.com/site/fjavierrubio67/).
```
...
- 當我們使用貝葉斯方法把專家給出的先驗概率，以及本次實驗給出的似然合並之後，（此處省略一萬字），獲得的事後概率認爲，心肌梗塞患者在家中立刻服藥沒有幫助或者甚至有害的概率是0.5%，也就是下圖中紅色的事後概率密度曲線下最右邊垂直虛線往右，曲線下面積，即比值比 OR 大於1的累計概率是0.5%：
...
```{r GREAT-sceptical, cache=TRUE, fig.height=3, fig.width=5, fig.cap='Sceptical Prior probablity density, no benifit or large treatment effect unlikely', fig.align='center', out.width='90%', echo=TRUE}
x <- seq(0, 1.5, by = 0.001)
...
```{r GREAT-sceptical-posterior, cache=TRUE, fig.height=3, fig.width=5, fig.cap='Sceptical Prior probablity density, adding likelihood (blue) from the GREAT trial, and posterior distribution (red).', fig.align='center', out.width='90%', echo=TRUE}
x <- seq(0, 1.5, by = 0.001)
y <- dlnorm(x, meanlog = 0.08, sdlog = 0.25) # use log Normal Distribution generator
z <- dlnorm(x, meanlog = log(0.47), sdlog = (-0.03+1.47)/(2*1.96))
...
即使專家的觀點樂觀且支持藥物治療的有效性，它仍然起到了把實驗數據給出的似然概率向更現實的方向（不那麼樂觀的方向）拉動了一點點，使用專家意見給出的事後比值比均值是0.73，值得關注的是，採用懷疑論觀點的先驗概率分布（上圖中使用OR=1作先驗概率的計算過程），也能給出相似的事後比值比均值 0.70，只是懷疑觀點的先驗概率分布使得事後概率分布中藥物可能無效甚至有害的概率變大了。這個分析展示了，一個對該藥物療效持懷疑觀點的人，和持樂觀觀點的人二者分析相同數據時很可能作出不同的結論，但是有一點是相同的，那就是傳統概率論統計學計算獲得的比值比，$\text{OR} = 0.47; 95\%\text{CI:} (0.23, 0.97), p = 0.04$不論與專家意見相比，還是和懷疑論者相比，都**過於樂觀了** (0.48 is too good to be true)。
...
```{r beta-distr-drug, echo=TRUE, fig.height=5, fig.width=7.5, fig.cap='Prior distribution for Drug Example', fig.align='center', out.width='90%', cache=TRUE}
# par(mfrow=c(2,3))
...
我們再回到新藥治療慢性疼痛的臨牀試驗上來，假設我們設計接納20名患者進入這個臨牀試驗。那麼我們可能想根據已有的經驗，來預測一下這進來的20名患者中藥物治療有效的人數。
...
# bugpath <- "C:/Users/owner/Documents/LSHTMlearningnote/" # in DELL CSS
...
# Step 1 check model
modelCheck(paste(bugpath, "backupfiles/drug-MCuniform.txt", sep = "")) 
# there is no data so just compile the model
modelCompile(numChains = 1) 
# There is no need to provide initial values as this is 
# a Monte Carlo forward sampling from a known distribution
...
# Step 1 check model
modelCheck(paste(bugpath, "backupfiles/standardnormalMC.txt", sep = "")) 
# there is no data so just compile the model
modelCompile(numChains = 1) 
# There is no need to provide initial values as this is 
# a Monte Carlo forward sampling from a known distribution
...
# Step 1 check model
modelCheck(paste(bugpath, "backupfiles/MCt.txt", sep = "")) 
# there is no data so just compile the model
modelCompile(numChains = 1) 
# There is no need to provide initial values as this is 
# a Monte Carlo forward sampling from a known distribution
...
# Step 1 check model
modelCheck(paste(bugpath, "backupfiles/MCcube.txt", sep = "")) 
# there is no data so just compile the model
modelCompile(numChains = 1) 
# There is no need to provide initial values as this is 
# a Monte Carlo forward sampling from a known distribution
...
# Step 1 check model
modelCheck(paste(bugpath, "backupfiles/MCdrugP29.txt", sep = "")) 
# there is no data so just compile the model
modelCompile(numChains = 1) 
...
#### PUT THE SAMPLED VALUES IN ONE R DATA FRAME:
chain.3 <- data.frame(P.crit = samplesSample("P.crit"), 
...
plot(density(chain.3$theta), main = "theta sample 50000", xlim = c(0,1),
     ylab = "P(theta)", xlab = "Probability of response", col = "red")

hist(chain.3$y.pred, main = "y.pred sample 50000", prob = TRUE,xlim = c(0, 40),
...
plot(chain.3$theta, main="", type="l", ylab="theta", 
...
```{r BayesianPractical0801, cache=TRUE, fig.width=8, fig.height=50, fig.cap='Density plots for parameters for hierarchical GLM about the odds of malaria regarding netbeds use in Gambia children.', fig.align='center', out.width='80%', message=TRUE, warning=FALSE, echo=TRUE}
# Step 1 check model
