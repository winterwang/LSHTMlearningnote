...
Mean_siblings <- Mean_siblings %>%
  mutate(# extract the random effect (EB) residuals at level 2
         ...
         # number of child for each mother
         Nchild = count(siblings$momid)[2]$freq, 
         # shrinkage factor 
         R = 315.7338^2/(315.7338^2 + (343.4572^2)/Nchild), 
...
ggplot(gcse_selected, aes(x = lrt, y = gcse)) + geom_blank() + 
...
```{r 4-level2-residuals, cache=TRUE, echo=FALSE, fig.height=5, fig.width=11, fig.cap='Q-Q plots of school level intercept and slope (standardized) residuals', fig.align='center', out.width='80%', message=FALSE, warning=FALSE}
...
**混合對稱模型 compound symmetry model**， 是常見的一種用於重復測量數據的模型，它是基於隨機截距模型的一種擴展模型。
...
也正是由於觀測值的協方差矩陣是如此地對稱，該模型被命名爲混合對稱模型 compound symmetric model。
...
```{r 5-level2-ress, cache=TRUE, echo=FALSE, fig.height=5, fig.width=11, fig.cap='Standardized cluster level residuals (intercept) from the compound symmetry model', fig.align='center', out.width='80%', message=FALSE, warning=FALSE, eval=FALSE}
# refit the model with lme
M_hei_ran <- lme(fixed = ht ~ age, random = ~ age | id, data = hei_long, method = "REML", na.action = na.omit)
# individual level standardized residuals
...
uhat_eb <- ranef(M_hei_ran, standard=TRUE)[,2] 
...
M_hei_ran <- lmer(ht ~ age + (age | id), data = hei_long, REML =TRUE)
uhat_eb <- HLMdiag::HLMresid(M_hei_ran, level = "id", type = "EB", standardize = TRUE)


ggthemr("fresh")

plot_u0hat <- HLMdiag::ggplot_qqnorm(uhat_eb$inte_st, line = "quantile") +
  theme(axis.text = element_text(size = 15),
  axis.text.x = element_text(size = 15),
  axis.text.y = element_text(size = 15)) +
  labs(y = "Level 2 cluster standardized residuals (EB) of intercept")  +
...
plot_u1hat <- HLMdiag::ggplot_qqnorm(uhat_eb$slop_st, line = "quantile") +
  ...
  axis.text.x = element_text(size = 15),
  axis.text.y = element_text(size = 15)) +
  labs(y = "Level 2 cluster standardized residuals (EB) of slope")  +
  theme(axis.title = element_text(size = 14), axis.text = element_text(size = 8))
