...
q00 <- quantile(x,0)
dens_obj <- density(x)
plot(dens_obj, xlim=c(0,20), frame.plot = FALSE, main=" ", yaxs="i",
     ylim=c(0,0.18), xlab="")
x1 <- max(which(dens_obj$x <= q05))
x2 <- min(which(dens_obj$x > q00))
x3 <- min(which(dens_obj$x >= q95))
x4 <- max(which(dens_obj$x <  q100))
with(dens_obj, polygon(x=c(x[c(x1,x1:x2,x2)]), y= c(0, y[x1:x2], 0), col="gray"))
with(dens_obj, polygon(x=c(x[c(x3,x3:x4,x4)]), y= c(0, y[x3:x4], 0), col="gray"))
