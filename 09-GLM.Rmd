...
Pearson 殘差的平方和，就是 Pearson 卡方統計量：
...
NHANES$gender[NHANES$gender == 1] <- "Male"
NHANES$gender[NHANES$gender == 2] <- "Female"
NHANES$gender <- factor(NHANES$gender, levels = c("Male", "Female"))
...
圖 \@ref(fig:stPearsonAge) 中靠近橫軸的藍色實線是 LOWESS 平滑曲線，它十分接近平直的橫線，也證明了 Pearson 標準化殘差值和年齡本身並無關聯。這同時也佐證了，將年齡以連續型共變量的形式放入本次邏輯迴歸模型中**並非不合理 (no unreasonable)**。
...
NHANES$stresPearson_bmi <- boot::glm.diag(Model_NH3)$rp       ## 可以獲取 standardized Pearson residuals
...
本次練習繼續沿用 NHANES 數據，此次練習我們把重點放在收集到的收縮期血壓數據上。假設定義收縮期血壓大於 140 $\text{mmHg}$ 者爲高血壓患者。
