...
對手裡的數據擬合一個GLM的過程，其實就是在該數據的條件下(given the data)，對模型參數 $\mathbf{\beta}$ 定義其對數似然 (log-likelihood)，並尋找能給出其極大值的那一系列極大似然估計 (maximum likelihood estimates, MLE) $\mathbf{\hat\beta}$ 的過程。
