...
```{r}
Model0 <- glm(low~factor(ht), data = lbw, family = binomial(link = "logit"))
summary(Model0); Epi::ci.exp(Model0)
```
...
Model1 <- lm(lwt ~ factor(ht), data = lbw)
summary(Model1)
```
...
summary(Model2); Epi::ci.exp(Model2)
```
...
```{r}
Model3 <- glm(low ~ factor(ht) + lwt, data = lbw, family = binomial(link = "logit"))
...
上面的分析結果，告訴我們，數據提供了足夠的證據證明母親孕前體重和是否有高血壓病史，在調整了彼此之後，仍然獨立地和嬰兒低出生體重的發生有相關性。這裏，我們可以下結論認爲，模型中假如母親孕前體重作爲混雜因子，是合情合理的。
