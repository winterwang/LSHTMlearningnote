...
$$
Y_i | x_i \sim N(\alpha + \beta x_i, \sigma^2s)
$$
...
此時， $\beta$ 的含義是，當保持 $z$ 不變時，$x$ 每增加一個單位時，$y$ 的變化量。用這個模型，我們默認 $z$ 保持不變的同時無論取值爲多少， $x, y$ 之間的關係是不會變化的，我們用這個模型來調整 (adjust) $z$ 的混雜效應 (confounding effect) (Section \@ref(confounding))。
