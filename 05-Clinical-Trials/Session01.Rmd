...
# Reconstructed Australian & ISIS-2 trial comparison table without external file dependency.
# This ensures the chapter can knit standalone from its own directory or via bookdown.

Trial    <- c("1st Australian", "1st Australian", "ISIS-2", "ISIS-2")
Metric   <- c("Deaths", "Risk ratio (95% CI)", "Deaths", "Risk ratio (95% CI)")
Treatment<- c("26 (9.8%)", "0.78 (0.48–1.27)", "791 (9.2%)", "0.77 (0.70–0.84)")
Control  <- c("32 (12.6%)", "", "1029 (12.0%)", "")
`p-value`<- c("p = 0.32", "", "p < 0.001", "")
tbl <- data.frame(Trial, Metric, Treatment, Control, `p-value`, check.names = FALSE)

caption_txt <- "Results from the 1st Australian and ISIS-2 trials for reducing mortality from post-MI"

if (!requireNamespace("kableExtra", quietly = TRUE)) {
  try(install.packages("kableExtra"))
}
library(knitr)
library(kableExtra)

kable(tbl, "html", align = "l", caption = caption_txt) %>%
