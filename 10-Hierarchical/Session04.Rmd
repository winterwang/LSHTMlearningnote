...
School_res1 <- HLMdiag::hlm_resid(MIX_coef, level = "school", type = "EB", standardize = TRUE)
...
qqnorm(School_res0$.ranef.intercept, 
       ylab = "", 
       main = "BLUP r.e level 2 (school) intercept residuals", 
       pch = 20); qqline(School_res0$.ranef.intercept, col=2)
qqnorm(School_res0$.ranef.lrt, 
       ylab = "", 
       main = "BLUP r.e level 2 (school) slope residuals", 
       pch = 20); qqline(School_res0$.ranef.lrt, col=2)
```
...
qqnorm(School_res1$.std.ranef.intercept, 
       ylab = "", 
       main = "BLUP r.e level 2 (school) intercept residuals", 
       pch = 20); qqline(School_res1$.std.ranef.intercept, col=2)
qqnorm(School_res1$.std.ranef.lrt, 
       ylab = "", 
       main = "BLUP r.e level 2 (school) slope residuals", 
       pch = 20); qqline(School_res1$.std.ranef.lrt, col=2)
```



```{r 4-level1sta-residuals, cache=TRUE, echo=FALSE, fig.height=5, fig.width=11, fig.cap='Histogram and Q-Q plots of elementary level (pupil) standardized residuals', fig.align='center', out.width='80%', message=FALSE, warning=FALSE}
...
y <- with(gcse, dnorm(x, mean(ehat), sd(ehat)))
lines(x, y, col = "darkgreen", lwd = 2)

qqnorm(gcse$ehat, 
       ylab = "EB level 1 (pupil) standardized residuals", 
       pch = 20); qqline(gcse$ehat, col=2, lwd = 1.5)
```
