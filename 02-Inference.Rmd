...
# 多個參數時的統計推斷 Inference with multiple parameters I
...
# 多個參數時的統計推斷 -- 子集似然函數 profile log-likelihoods
...
3. 證明參數 $\theta$ 的子集對數似然是 $\ell_p(\theta|r) = n\text{log}\theta - 2n \text{log}(\theta\cdot r + 1)$，且 $text{MLE}$ 是 $\hat\theta = \frac{1}{r}$
...
7. 一個**“臨牀醫生”**來找你問了這樣的一個常見的問題：當我們使用 $t$ 檢驗的時候，爲什麼前提假設是數據服從 **正態分佈**? 而不使用**服從 $t$ 分佈** 這樣的前提條件？
