...
這公式很直觀，因爲當觀察數據更加支持 $H_1$ 時 ($L_{H_1}$ 更大)，$H_0$ 的可能性相對更小，就更應該被拒絕。而且，由於似然比越小，他的對數就越小，實際計算是我們常使用對數似然比：$\ell_{H_0}-\ell_{H_1}$。
...
重新再考慮符合假設：$H_0: \theta=\theta_0\;\text{v.s.}\;H_1: \theta>\theta_0$ 假如存在一個總是可用的最佳檢驗統計量，用 $T$ 來標記 (或 $T(x)$)， 這個統計量足夠大時，我們拒絕 $H_0$。 別忘了我們還要給事先固定好的顯著性水平 $\alpha$ 定義與之相關的判別區域：
...
```{r fig.height=4, fig.width=5, message=FALSE, warning=FALSE, echo=FALSE, cache=TRUE}
x <- rchisq(1000000, 5)
q100 <- quantile(x, 1)
q95 <- quantile(x, .95)
q05 <- quantile(x, 0.05)
...
plot(dens, xlim=c(0,20), frame.plot = FALSE, main=" ", yaxs="i",
     ylim=c(0,0.18),xlab = expression(paste("(T|", theta[0], ")")))
...
axis(1, at=c(1.17,11.17), labels = c("t1","t2"))
```
...
1. $T\geqslant t2$ 其中， $Prob(T\geqslant t2|H_0)=p1$;
2. $T\leqslant t1$ 其中， $Prob(T\leqslant t1|H_0) =p1$
...
$p1=Prob(\bar{X}\geqslant7.76|H_0)=Prob(Z+5\geqslant7.76)\\=Prob(Z\geqslant2.76)=0.003\\
\Rightarrow p=2\times p1=0.006$
